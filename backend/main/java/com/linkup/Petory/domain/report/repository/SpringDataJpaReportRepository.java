...
public interface SpringDataJpaReportRepository extends JpaRepository<Report, Long> {
    ...
    /**
     * 신고 목록 projection 페이징 조회 (관리자용).
     * reporter/handledBy를 통째로 로딩하던 것을 idx/username 2컬럼만 SELECT하도록 축소하고,
     * 전건 인메모리 조회 대신 DB 페이징으로 바꿔 데이터 누적에도 조회량이 일정하다.
     * (reportCount는 소비처가 없어 채우지 않음 — null)
     */
