...
public class ConditionalOAuth2TokenResponseClient 
    ...
    private OAuth2AccessTokenResponseClient<OAuth2AuthorizationCodeGrantRequest> getDefaultClient() {
        if (defaultClient == null) {
            // Spring Security 6.x의 기본 클라이언트
            // Nimbus 기반 클라이언트 사용 (deprecated 경고 무시)
            @SuppressWarnings("deprecation")
            var client = new org.springframework.security.oauth2.client.endpoint.DefaultAuthorizationCodeTokenResponseClient();
