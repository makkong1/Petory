 ...
 * 일반 사용자용 펫코인 컨트롤러
 *
 * 주의: 현재는 실제 결제 시스템(PG) 연동 없이 테스트 충전 기능을 사용합니다. 실제 운영 시에는 이 충전 엔드포인트를 PG 연동으로
 * 대체하거나, 별도의 결제 서비스로 분리하여 구현해야 합니다.
 */
@Slf4j
@RestController
...
public class PetCoinController {
    ...
    /**
     * 현재 사용자 거래 내역 조회 (DB 페이징) [리팩토링] 메모리 페이징 → DB 페이징, Page 응답 형식
     */
     ...
     * 코인 충전
     *
     * 현재는 실제 결제 시스템 연동 없이 시뮬레이션으로 처리합니다. 실제 운영 시에는 이 메서드를 PG 연동으로 대체하거나, 별도의 결제
     * 서비스로 분리하여 구현해야 합니다.
     */
    @PostMapping("/charge")
