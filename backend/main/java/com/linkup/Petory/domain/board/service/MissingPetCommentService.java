...
public class MissingPetCommentService {
     ...
     * /api/missing-pets/{id}/comments?page={page}&size={size} - 생성일 기준 오름차순 정렬
    ...
    public MissingPetCommentPageResponseDTO getCommentsWithPaging(Long boardId, int page, int size) {
        ...
        Page<MissingPetComment> commentPage = commentRepository
                .findByBoardIdAndIsDeletedFalseOrderByCreatedAtAsc(boardId, pageable);
        ...
        List<MissingPetCommentDTO> commentDTOs = comments.stream()
                .map(comment -> {
                    ...
                    List<FileDTO> attachments = filesByCommentId.getOrDefault(comment.getIdx(),
                            List.of());
    ...
    /**
     * 댓글 목록 조회 (페이징 없음 - 하위 호환성) 엔드포인트: GET /api/missing-pets/{id}/comments -
     * 생성일 기준 오름차순 정렬 - 삭제된 댓글 제외 - 각 댓글의 파일 정보 포함 - 댓글 파일 배치 조회 (N+1 문제 해결)
     */
    ...
    /**
     * 댓글 작성 엔드포인트: POST /api/missing-pets/{id}/comments - 댓글 이미지 업로드 지원 - 목격
     * 위치(주소, 좌표) 저장 가능 - 게시글 작성자에게 알림 발송 (비동기 처리)
     */
    @Transactional
    ...
    /**
     * 댓글 삭제 (소프트 삭제) 엔드포인트: DELETE
     * /api/missing-pets/{boardId}/comments/{commentId} - 게시글과 댓글의 연관 관계 검증
     */
    @Transactional
    ...
    /**
     * 게시글의 댓글 수 조회 (COUNT 쿼리 사용 - N건 로드 방지) [리팩토링] N건 전체 조회 + size() →
     * countByBoardAndIsDeletedFalse 1 COUNT 쿼리
     *
     * @param board 게시글 엔티티
     * @return 댓글 수
     */
    ...
    /**
     * 게시글의 모든 댓글 소프트 삭제 [리팩토링] N건 루프 save → 배치 UPDATE 1회 (1 SELECT + N UPDATE →
     * 1 UPDATE)
     *
     * @param board 게시글 엔티티
     */
    ...
    /**
     * 댓글 DTO 매핑 (파일 정보 포함) 단일 댓글 조회 시 사용
     */
    ...
    /**
     * 실종제보 댓글 알림 발송 (비동기 처리) 알림 발송 실패 시에도 댓글 작성은 성공하도록 분리
     */
    @Async
