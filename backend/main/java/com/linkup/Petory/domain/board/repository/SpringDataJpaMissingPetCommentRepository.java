...
public interface SpringDataJpaMissingPetCommentRepository extends JpaRepository<MissingPetComment, Long> {
    ...
    @Query("SELECT mc FROM MissingPetComment mc JOIN FETCH mc.user u WHERE mc.board = :board AND mc.isDeleted = false AND u.isDeleted = false AND u.status <> com.linkup.Petory.domain.user.entity.UserStatus.BANNED ORDER BY mc.createdAt ASC")
    ...
    @Query("SELECT COUNT(mc) FROM MissingPetComment mc JOIN mc.user u " +
           "WHERE mc.board = :board AND mc.isDeleted = false AND u.isDeleted = false AND u.status <> com.linkup.Petory.domain.user.entity.UserStatus.BANNED")
    long countByBoardAndIsDeletedFalse(@Param("board") MissingPetBoard board);

    @RepositoryMethod("실종 댓글: 사용자별 목록 조회")
    @Query("SELECT mc FROM MissingPetComment mc JOIN FETCH mc.board b JOIN FETCH b.user bu JOIN FETCH mc.user u WHERE mc.user = :user AND mc.isDeleted = false AND u.isDeleted = false AND u.status <> com.linkup.Petory.domain.user.entity.UserStatus.BANNED ORDER BY mc.createdAt DESC")
    ...
    @Query(value = "SELECT mc FROM MissingPetComment mc JOIN FETCH mc.user u WHERE mc.board.idx = :boardId AND mc.isDeleted = false AND u.isDeleted = false AND u.status <> com.linkup.Petory.domain.user.entity.UserStatus.BANNED ORDER BY mc.createdAt ASC",
           countQuery = "SELECT COUNT(mc) FROM MissingPetComment mc JOIN mc.user u WHERE mc.board.idx = :boardId AND mc.isDeleted = false AND u.isDeleted = false AND u.status <> com.linkup.Petory.domain.user.entity.UserStatus.BANNED")
