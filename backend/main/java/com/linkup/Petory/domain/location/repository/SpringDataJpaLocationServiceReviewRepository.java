...
public interface SpringDataJpaLocationServiceReviewRepository extends JpaRepository<LocationServiceReview, Long> {
    ...
    /**
     * 서비스별 리뷰 목록 projection 페이징 조회.
     * user를 통째로(JOIN FETCH) 로딩하던 것을 idx/username 2컬럼만 SELECT하도록 축소하고,
     * 전건 반환 대신 DB 페이징으로 바꿔(장소당 리뷰 누적과 무관하게 조회량 일정) '더보기' 누적 방식으로 소비한다.
     * 목록에선 serviceName을 쓰지 않으므로 service를 조인하지 않고 null로 둔다.
     */
