...
public class KakaoMapService {
    ...
    public List<KakaoPlaceDTO.Document> searchPlaces(String keyword, String category, String region, int maxResults) {
        ...
        try {
            while (allPlaces.size() < maxResults) {
                URI uri = UriComponentsBuilder.fromUriString(KAKAO_LOCAL_SEARCH_URL)
                        .queryParam("query", keyword)
                        .queryParam("category_group_code", category != null ? category : "")
                        .queryParam("region", region != null ? region : "")
            ...
            log.info("카카오맵 API로 {}개의 장소를 검색했습니다.", allPlaces.size());
            return allPlaces.subList(0, Math.min(allPlaces.size(), maxResults));

        } catch (Exception e) {
            log.error("카카오맵 API 호출 중 오류 발생: {}", e.getMessage(), e);
            return allPlaces;
        }
    }
     ...
     * 여러 키워드로 병렬 검색
     */
    public List<KakaoPlaceDTO.Document> searchPlacesByMultipleKeywords(List<String> keywords, String category, String region, int maxResultsPerKeyword) {
        List<KakaoPlaceDTO.Document> allPlaces = new ArrayList<>();
