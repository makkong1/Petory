...
// @Test
// @DisplayName("문제 1: Object[] 파싱 로직 중복 - mapReactionCounts와
// getReactionCountsBatch에서 동일한 로직 사용")
...
// when(attachmentFileService.getAttachmentsBatch(eq(FileTargetType.BOARD),
// anyList()))
...
// @Test
// @DisplayName("문제 2: 반응 카운트 설정 로직 중복 - mapReactionCounts와
// mapBoardsWithReactionsBatch에서 동일한 로직")
...
// @Test
// @DisplayName("문제 3: 첨부파일 설정 로직 중복 - mapAttachmentInfo와
// mapBoardsWithReactionsBatch에서 동일한 로직")
...
// verify(attachmentFileService).getAttachmentsBatch(eq(FileTargetType.BOARD),
// anyList());
// }

// @Test
// @DisplayName("문제 4: 단일/배치 조회 분리 - mapBoardWithDetails와
// mapBoardsWithReactionsBatch가 다른 방식으로 구현")
...
// // When: 단일 조회 (mapBoardWithDetails -> mapReactionCounts, mapAttachmentInfo
// 사용)
// BoardDTO singleResult = boardService.getBoard(boardId, null);
...
// ContentStatus status = i % 2 == 0 ? ContentStatus.ACTIVE :
...
// when(boardConverter.toDTO(any(Board.class))).thenAnswer(new
// Answer<BoardDTO>() {
// @Override
...
// System.out.println("실제 필요한 게시글 수: " + result.getBoards().size() + "개 (필터링
// 후)");
// System.out.println("불필요한 데이터 조회: " + (1000 - result.getBoards().size()) +
// "개");
...
// System.out.println("비효율성 비율: " + ((1000.0 - result.getTotalCount()) / 1000.0
// * 100) + "%");
