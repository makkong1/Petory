...
createNotification()
  ...
  → SSE push
  → FCM push (토큰 없으면 early return — 웹 사용자는 no-op)
```
...
2. `saveIfConfident()` 저장 성공 후 MEDICAL+HIGH인 경우에만 알림 생성  
   → **저장 트랜잭션과 반드시 분리해야 한다.** `saveIfConfident()` 내부 `@Transactional` 컨텍스트에서 `createNotification()`을 직접 호출하면 SSE가 DB 커밋 전에 나간다.  
   → `@Async`만으로는 after-commit이 보장되지 않는다. 올바른 방법: `saveIfConfident()` 내부에서 `ApplicationEventPublisher`로 `SignalSavedEvent`를 발행하고, 별도 `@TransactionalEventListener(phase = AFTER_COMMIT)` 핸들러에서 알림을 생성한다.

3. `Navigation.js` `handleNotificationClick()` 에 `PET_HEALTH_ALERT` 케이스 추가  
   → 라우팅은 `notification.relatedType`이 아니라 **`notification.type === 'PET_HEALTH_ALERT'`** 기준으로 처리한다.  
   → 알림 페이로드: `type = PET_HEALTH_ALERT`, `relatedType = PET_INTENT_SIGNAL`, `relatedId = signalId`  
   → 클릭 시 `setActiveTab('unified-map')` 이동.  
   → 동물병원 카테고리 자동 선택은 `Navigation.js`에서 직접 불가 — `CustomEvent` 또는 상위 상태 전달이 추가로 필요하다 (**추가 구현 필요**).

#### 현재 Navigation.js 라우팅 상태 (수정 필요)

```js
// Navigation.js:215 — 현재 (relatedType 기준)
...
// 수정 후 — type 기준으로 추가
  ...
  // TODO: CustomEvent로 동물병원 카테고리 선택 전달
}
```
