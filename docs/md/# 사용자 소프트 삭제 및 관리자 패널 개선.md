...
> **[2026-07] 업데이트**: 탈퇴 계정의 id/username/nickname/email이 UNIQUE 제약을
> 그대로 점유해 영구히 재사용 불가능한 문제가 발견되어, 계정 복구 기능은
> 제거하고 `softDelete()` 시점에 해당 필드를 즉시 익명화하도록 재설계했다.
> 탈퇴는 되돌릴 수 없으며 재가입만 가능하다.
...
- `DELETE /api/admin/users/{id}` - 계정 탈퇴 처리 (복구 불가)
