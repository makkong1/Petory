...
- ~~권한 검증: 댓글 삭제는 작성자/관리자 확인이 없음~~ → **[개선 완료]** `Authentication`으로 요청자 신원 확인, 작성자·관리자 여부 검증 추가
- 권한 검증: `createCareRequest`·댓글 작성·리뷰 작성은 요청 바디의 `userId`를 신뢰 — JWT 기반 검증으로 개선 여지 있음
