...
**결론**: Petory는 대부분 3NF를 준수하되, 조회 성능을 위해 **선택적 비정규화**를 적용한 구조로 설계되어 있음.

---

## 5. 동기화 실측 결과 (2026-07-13)

위 결론의 마지막 숙제였던 *"집계/캐시 필드의 동기화 로직이 실제로 보장되는가"* 를 실제 DB에 쿼리를 던져 확인했다.
전체 수치·근거·재사용 가능한 점검 쿼리는 **[evidence/denormalization-consistency-2026-07-13.md](evidence/denormalization-consistency-2026-07-13.md)** 참고.

### 요약

| 항목 | 판정 |
|------|------|
| **동기화 코드** | ✅ **정합.** ReactionService·CommentService·MeetupRepository 모두 증감이 대칭이고, 자식 행 변경과 같은 트랜잭션 안의 원자적 UPDATE다. |
| **현재 DB 데이터** | ❌ **오염됨.** `like_count` 168건, `comment_count` 548건, `current_participants` 2,791건이 원본 집계와 불일치. |
| **원인** | 코드 버그가 아니라 **대량 생성된 더미 데이터**. `view_count` 총합 5,154,619 vs `board_view_log` 17행 — 실사용으로는 불가능한 수치가 결정적 근거. |
| **스키마 구조** | ✅ **변경 불필요.** 3NF 준수 + 의도적 비정규화라는 위 판정을 유지한다. |

### 진짜 문제

**드리프트를 탐지하거나 복구할 장치가 없다.** 데이터가 어긋나 있다는 사실 자체를 아무도 몰랐던 것이 그 증거다.
반정규화 카운터는 원자적 UPDATE 만으로 영구히 보장되지 않는다 — 배치 삽입·수동 SQL 등 앱을 우회하는 경로가 운영 중 반드시 생기기 때문이다.
**정합성을 지키는 유일한 방법은 주기적으로 원본과 대조하는 것이고, 그 쿼리는 evidence 문서 §4에 있다.**

⚠️ `pet_coin_balance` 는 거래 이력이 있는 유저가 2명뿐이라 **이번 측정으로 안전하다고 말할 수 없다.** 돈이 걸린 필드이므로 별도 검증이 필요하다.
