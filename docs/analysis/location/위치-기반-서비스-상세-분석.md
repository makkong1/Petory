...
1. FULLTEXT 인덱스가 실제로 `name, description, category1, category2, category3`에 생성되어 있는지
2. 검색 결과가 기대대로 나오는지 (예: "동물병원" 검색 시 description에 "동물병원"이 포함된 항목도 검색되는지)
...
void 키워드_검색_테스트() {
    ...
    // when
    List<LocationServiceDTO> results = service.searchLocationServicesByKeyword(keyword, null, null);
    
    // then
    assertThat(results).isNotEmpty();
    // name에 포함된 항목
    assertThat(results).anyMatch(r -> r.getName().contains(keyword));
    // description에 포함된 항목 (FULLTEXT 검색 확인)
    assertThat(results).anyMatch(r -> 
        r.getDescription() != null && r.getDescription().contains(keyword));
    // category에 포함된 항목
    assertThat(results).anyMatch(r -> 
        r.getCategory1().contains(keyword) || 
        r.getCategory2().contains(keyword) || 
        r.getCategory3().contains(keyword));
}
```

---

### 문제 4: 하이브리드 전략의 일관성 문제 ⚠️ **보통**
...
**명확한 검색 전략 분리**:
- 초기 로드: 위치 기반 검색 (5km 반경)
- 지역 선택: 항상 백엔드 재요청 (일관성 유지)
- 키워드 검색: 항상 백엔드 요청

```javascript
// 지역 선택 시 항상 백엔드 재요청
const handleRegionSelect = async (sido, sigungu, eupmyeondong) => {
...
| 🔴 High | 프론트엔드 검색 로직 단순화 | 가독성 향상, 유지보수 용이 | ⏳ 진행 중 |
| 🟡 Medium | 키워드 검색 품질 검증 | 검색 품질 향상 | ⏳ 대기 |
| 🟡 Medium | 하이브리드 전략 일관성 개선 | 사용자 경험 향상 | ⏳ 대기 |
| 🟢 Low | 상태 관리 개선 | 코드 품질 향상 | ⏳ 대기 |
