...
## 5. 채팅방 목록 N+1 해결 — 배치 조회 (Chat `ConversationService.getMyConversations`, 로그인 직후 로드; 실측 측정 완료)

> ⚠️ **주의**: 이 쿼리는 **Chat 도메인** `getMyConversations`이며, 로그인 직후 프론트가 채팅방 목록을 로드하는 시나리오에서 측정됐다. **인증/로그인(AuthService) 코드 자체가 아니다** — 면접에서 "로그인 쿼리"라고 표현하면 코드와 어긋난다.
...
### 말할 내용
> "로그인 직후 프론트가 호출하는 **채팅방 목록 조회 API(Chat `ConversationService.getMyConversations`)** 에서 N+1 문제가 발생해 채팅방 10개 기준 21개 쿼리가 실행되고 이 API 응답 시간이 305ms나 걸렸습니다. (인증/로그인 처리 자체가 아니라 채팅방 목록 조회 쿼리입니다.) 채팅방 ID 목록을 먼저 추출하고 IN 절로 배치 조회하도록 변경했고, 최신 메시지 1건만 서브쿼리로 가져오도록 바꿨습니다. 결과적으로 쿼리는 4개로 고정되고 이 API 응답 시간은 55ms로 줄었습니다. 채팅방이 100개로 늘어도 쿼리는 4개로 유지됩니다."
...
1. **문제 발견** (20초)
   - "로그인 직후 호출되는 채팅방 목록 API(Chat `getMyConversations`)에서 채팅방 10개 기준 21개 쿼리와 305ms 응답 시간을 측정했습니다 (인증 코드가 아니라 Chat 조회 쿼리)."
