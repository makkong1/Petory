...
**현재 문제**: 채팅방 생성 실패 시 모임 생성은 성공

**코드 위치**: `MeetupService.createMeetup()` (line 86-105)
...
**문제점**:
- 같은 `@Transactional` 메서드 안에 있지만, try-catch로 예외를 잡아서 트랜잭션이 롤백되지 않음
- 채팅방 생성 실패 시 모임만 생성되고 채팅방은 없음 → 데이터 일관성 문제
- 사용자는 모임을 생성했지만 채팅방이 없어서 혼란 발생 가능
...
public MeetupDTO createMeetup(...) {
    ...
    // 채팅방 생성 실패 시 예외를 그대로 전파하여 트랜잭션 롤백
    conversationService.createConversation(...);
    conversationService.setParticipantRole(...);
...
```
- **장점**: 데이터 일관성 보장
- **단점**: 채팅방 생성 실패 시 모임도 생성 실패

#### 방안 2: 별도 트랜잭션으로 분리 (권장)
```java
@Transactional
public MeetupDTO createMeetup(...) {
    Meetup savedMeetup = meetupRepository.save(meetup);
    
    // 채팅방 생성을 별도 트랜잭션으로 분리
    try {
        createChatRoomAsync(savedMeetup, organizer);
    } catch (Exception e) {
        log.error("채팅방 생성 실패 (비동기): ...", e);
        // 모임은 이미 생성됨, 채팅방은 나중에 재시도 가능
    }
...
public void createChatRoomAsync(Meetup meetup, Users organizer) {
    conversationService.createConversation(...);
    conversationService.setParticipantRole(...);
}
```
- **장점**: 모임 생성과 채팅방 생성 독립적 처리, 재시도 가능
- **단점**: 구현 복잡도 증가

#### 방안 3: 현재 방식 유지 + 재시도 메커니즘
- 채팅방 생성 실패 시 로그 기록
- 별도 스케줄러로 채팅방 없는 모임 감지 및 재시도
- **장점**: 기존 로직 유지, 점진적 개선
- **단점**: 일시적 데이터 불일치 허용

**권장 사항**: 방안 2 (별도 트랜잭션) 또는 방안 1 (롤백) 중 선택
...
**문제**: 복잡한 읽음 처리 로직에서 트랜잭션 범위가 넓음
- **개선**: 읽음 처리 로직 단순화 및 배치 처리
...
- **개선**: 커서 기반 페이징으로 대량 메시지 처리 성능 향상 (이미 `getMessagesBefore()` 구현됨)
