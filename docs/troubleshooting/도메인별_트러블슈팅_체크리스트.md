...
**위치**: `ChatMessageService.sendMessage()`
...
### ✅ 해결됨: markAsRead 전체 메시지 대량 조회

**위치**: `ChatMessageService.markAsRead()`

**해결 내용**: 전체 메시지 조회 + Java 스트림 필터링 → 제거 완료.  
현재는 `setUnreadCount(0)` + `setLastReadMessage` + `participantRepository.save()` 만으로 동작.  
`MessageReadStatus` 기록 로직도 제거됨 (주석으로 사유 명시).

### ⚠️ 미해결: 인덱스 미적용

**위치**: `chatmessage`, `conversationparticipant` 테이블

`docs/migration/db/indexes.sql` 기준 FULLTEXT(content)만 적용됨. 아래 4건 미적용:

```sql
-- 메시지 조회 성능 향상 (가장 중요)
CREATE INDEX idx_chat_message_conversation_created ON chatmessage(conversation_idx, created_at DESC);
CREATE INDEX idx_chat_message_sender ON chatmessage(sender_idx);

-- 참가자 조회 성능 향상
CREATE UNIQUE INDEX idx_conversation_participant_unique ON conversationparticipant(conversation_idx, user_idx);
CREATE INDEX idx_conversation_participant_status ON conversationparticipant(status, unread_count);
```

**우선순위**:
1. **높음**: `idx_chat_message_conversation_created` — 가장 자주 사용되는 커서 페이징 쿼리
2. **높음**: `idx_conversation_participant_unique` — 중복 방지 + 조회 성능
3. **중간**: `idx_chat_message_sender`, `idx_conversation_participant_status`

### ⚠️ 미해결: 프론트엔드 markAsRead 과호출

**위치**: `frontend/src/components/Chat/ChatRoom.js`

`markAsRead` 호출 지점 5곳 (디바운싱 없음):
- 메시지 최초 로드 후 (line 66)
- WebSocket 새 메시지 수신마다 (line 180) — 빠른 채팅 시 연속 호출
- 이미지 전송 후 2회 연속 (line 253, 258)
- 텍스트 전송 후 2회 연속 (line 297, 302)

**개선 방향**: WebSocket 수신 핸들러에 debounce(300ms) 적용, 전송 후 이중 호출 제거.
...
### ✅ 해결됨: 탈퇴한 사용자(Soft Delete)의 닉네임/username/email 재사용 불가 문제

**위치**: `SpringDataJpaUsersRepository.findByNickname()`, `findByUsername()`, `findByEmail()`

**해결 내용**: 세 메서드 모두 `@Query`로 교체하여 `AND (u.isDeleted = false OR u.isDeleted IS NULL)` 조건 추가.  
`findByNicknameOrUsernameOrEmail` 통합 중복 체크 메서드도 동일 조건 포함.
...
- ❌ `getAllBoards()` 캐시 주석 처리 + 해당 메서드 자체가 dead code  
  (Controller는 `getAllBoardsWithPaging`을 호출, `getAllBoards(List<>)` 반환 버전은 사용되지 않음)

**현행 상황**:
- `getAllBoards` 메서드 제거 또는 `@Deprecated` 처리 필요
- `boardDetail` 캐시는 Spring 기본 캐시 (로컬 메모리) 사용 중 — 서버 재시작 시 소실, 멀티 인스턴스 불일치

**개선 방향**:
- `boardDetail` 캐시를 Redis(`@Cacheable` + `RedisCacheManager`) 로 전환
- `getAllBoards` dead code 제거

### ⚠️ 미해결: 조회수 증가 Race Condition

**위치**: `BoardService.shouldIncrementView()`

**현재 구현**:
- `existsByBoardAndUser(board, viewer)` → 없으면 `BoardViewLog.save()` TOCTOU 패턴
- 동시 조회 시 두 요청 모두 `existsByBoardAndUser = false` 를 받고 중복 증가 가능

**개선 방향**:
- `board:{boardId}:viewers` Redis Set에 userId 저장 (TTL 24h) → `SADD` 반환값으로 최초 조회 판단
- `BoardViewLog` DB 기록 불필요 → 테이블 크기 문제 함께 해결
- 조회수 증가를 `@Async`로 비동기 처리하여 응답 지연 제거
