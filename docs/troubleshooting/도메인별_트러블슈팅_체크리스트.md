...
### ✅ 이미 개선됨: 배치 조회로 N+1 문제 해결

**위치**: `BoardService.mapBoardsWithReactionsBatch()` (line 435-472)

### ⚠️ 잠재적 문제: 조회수 증가 Race Condition

**위치**: `BoardService.getBoard()` (line 207-218)

**현재**: 단일 트랜잭션에서 조회수 증가
- **개선**: 비동기 처리 또는 배치 업데이트 고려
...
CREATE FULLTEXT INDEX idx_board_title_content ON board(title, content); -- 검색용
...
**현재**: 검색 시 Full-Text Search 사용 (line 387)
- **개선**: 검색 결과 정확도 향상을 위한 알고리즘 적용 (TF-IDF, BM25 등)
...
### ⚠️ 가장 심각한 문제: 중복 신고 체크 Race Condition

**위치**: `ReportService.createReport()` (line 65-70)
...
// ... 
Report saved = reportRepository.save(report); // ⚠️ Race Condition!
```

**시나리오**:
- 같은 사용자가 동시에 같은 대상을 2번 신고 시도
- 둘 다 중복 체크 통과
- 둘 다 신고 성공 → 중복 신고 발생!

**해결 방안**:

#### 6.1 DB 레벨 Unique 제약조건
```sql
CREATE UNIQUE INDEX idx_report_unique 
ON report(target_type, target_idx, reporter_idx);
```

#### 6.2 Pessimistic Lock 적용
```java
@Lock(LockModeType.PESSIMISTIC_WRITE)
