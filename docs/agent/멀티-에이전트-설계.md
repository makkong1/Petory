...
- [x] ReportAssistAgentService (Ollama 연동)
- [x] GET /api/admin/reports/{id}/assist
- [x] ReportAssistSuggestion DTO
- [x] ReportDetailModal AI 보조 UI (제안 불러오기, 제안 적용, 관리자 메모 반영)
- ⚠️ 한글 응답(summary, reasoning)이 어색할 수 있음 (모델·프롬프트에 따라)
...
- **구현 시**: 추천 결과를 **10개 정도로만** 제한하여 LLM 호출·토큰 비용 절감.

### 3.4 추천 기준

반려동물 플랫폼에 맞춘 추천 기준 (중요도 순):

| 순위 | 기준 | 설명 | DTO 필드 |
|------|------|------|----------|
| 1 | 거리 | 가까울수록 유리 | `distance` |
| 2 | 반려동물 친화 | 반려동물 동반 가능 여부 | `petFriendly` |
| 3 | 평점 | 사용자 평가 | `rating` |
| 4 | 리뷰 수 | 리뷰가 많을수록 신뢰도 상승 | `reviewCount` |
| 5 | 카테고리 일치 | 사용자 선택 카테고리와 일치 | `category` |

**보조 기준 (선택)**: `parkingAvailable`, `petSize`/`petRestrictions`, `indoor`/`outdoor`, `operatingHours`/`closedDay`

**가중치 예시 (나중에 세밀 조정 시)**: 거리 30%, 반려동물 친화 25%, 평점 25%, 리뷰 수 10%, 카테고리 10%

**프롬프트 예시**: "다음 장소 목록에서 반려동물과 함께 가기 좋은 상위 10개를 추천해라. 고려 기준(중요도 순): 1) 거리 2) 반려동물 친화 3) 평점 4) 리뷰 수 5) 카테고리. 각 장소별 1줄 추천 이유를 한국어로 달아라."
...
- **권한**: 로그인만 (이메일 인증 생략)
- **호출 방식**: **클릭 방식**으로 구현. 사용자가 "AI 추천" 버튼을 눌렀을 때만 호출. 위치 변경 시 자동 호출은 하지 않음 (Ollama 메모리·비용 절감).
...
- **입력 처리**: `latitude`, `longitude`, `radius`가 없으면 400 반환 또는 사용자 위치/지도 중심 좌표 사용. 프론트에서 "AI 추천" 클릭 시 현재 지도 중심 또는 사용자 GPS를 넘기도록 정의.
- **이메일 인증**: 생략. 로그인만 확인 (`/api/**` 기본 인증).
- **DTO 확장**: `LocationServiceDTO`에 `recommendationReason` (String) 필드 추가.
- **reviewCount**: `LocationServiceConverter`에서 `reviewCount`를 설정하지 않을 수 있음. 없으면 LLM 프롬프트에서 "리뷰 수가 있으면 참고, 없으면 무시"로 처리.
