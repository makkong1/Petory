...
특징:
- 기존 키 수정 시 `description`도 함께 갱신 가능
...
| Method | URL | 설명 |
|--------|-----|------|
| GET | `/api/admin/boards/paging` | 게시글 목록 (`status`, `deleted`, `category`, `q`) |
...
| POST | `/api/admin/location-services/import-public-data` | CSV 업로드 임포트 |
...
| Method | URL | 설명 |
|--------|-----|------|
| GET | `/api/admin/statistics/daily` | 일별 통계 |
| GET | `/api/admin/statistics/weekly` | 주별 통계 |
| GET | `/api/admin/statistics/monthly` | 월별 통계 |
| GET | `/api/admin/statistics/summary` | 오늘 스냅샷 |
| POST | `/api/admin/statistics/backfill` | 통계 백필 |
