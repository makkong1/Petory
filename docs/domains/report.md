...
| `UserNotFoundException` | 신고자/관리자 조회 실패 |
...
| 엔드포인트 | Method | 설명 |
|-----------|--------|------|
| `/api/reports` | POST | 신고 접수 (인증 필요, ReportValidationException, ReportConflictException, ReportTargetNotFoundException, ReportForbiddenException) |

#### AdminReportController (`/api/admin/reports`, ADMIN/MASTER)
| 엔드포인트 | Method | 설명 |
|-----------|--------|------|
| `/api/admin/reports` | GET | 신고 목록 조회 (targetType, status 파라미터, 신고 횟수 기준 정렬) |
| `/api/admin/reports/{id}` | GET | 신고 상세 조회 (대상 미리보기 포함) |
| `/api/admin/reports/{id}/assist` | GET | AI 보조 (요약·심각도·조치 제안, Ollama, 실패 시 null) |
| `/api/admin/reports/{id}/handle` | POST | 신고 처리 (ReportHandleRequest, ReportValidationException.statusRequired, ReportNotFoundException) |
