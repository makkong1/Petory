...
- **현재 백엔드에는** `CLOSED`/`COMPLETED`로 바꾸는 **스케줄러·서비스 로직이 없음** (자동 마감/종료는 미구현). 필요 시 `updateMeetup` 등으로 확장 가능.
- `findNearbyMeetups` 네이티브 쿼리는 **`COMPLETED`가 아닌** 모임만 조회 (`status`가 null이면 통과).

### 2.3 위치 기반 검색

**반경 기반 검색** (`GET /api/meetups/nearby`, `radius` km, 기본 **5.0**):
- `findNearbyMeetups`: Bounding Box + Haversine(6371km)로 필터·정렬
...
| `updateMeetup()` | 모임 수정 | `findByIdWithDetails()`, MeetupNotFoundException, 선택적 필드 업데이트 (**주최자 검증 없음**) |
| `deleteMeetup()` | 모임 삭제 | MeetupNotFoundException, Soft Delete (**주최자 검증 없음**) |
| `getAllMeetups()` | 모든 모임 조회 | `findAllNotDeleted()`, JOIN FETCH로 N+1 방지 |
| `getMeetupById()` | 특정 모임 조회 | `findByIdWithDetails()`, MeetupNotFoundException, organizer·participants 포함 |
| `getNearbyMeetups()` | 반경 기반 모임 조회 | `findNearbyMeetups()`, Bounding Box + Haversine, radius 기본값 5.0km, @Timed |
...
| `getAvailableMeetups()` | 참여 가능한 모임 조회 | `findAvailableMeetups()`, 최대 인원 미만·미래 날짜 |
...
| `MeetupForbiddenException` | `organizerCannotCancel()`: 주최자 참가 취소 시도 |
...
**구현 참고**: `updateMeetup` / `deleteMeetup`은 서비스에서 **주최자 여부를 검사하지 않음**(보안·정책은 프론트/추가 가드 검토).
...
| `/api/meetups/{meetupIdx}` | PUT | 모임 수정 (MeetupNotFoundException, **주최자 검증 없음**) | `{"meetup": {...}, "message": "..."}` |
| `/api/meetups/{meetupIdx}` | DELETE | 모임 삭제 (MeetupNotFoundException, **주최자 검증 없음**) | `{"message": "..."}` |
...
- **상태**: `MeetupStatus` enum은 정의되어 있으나 **자동 전환 로직은 현재 없음**
