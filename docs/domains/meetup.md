...
- `SpringDataJpaMeetupRepository.findNearbyMeetupIds`: `geo_point` 공간 컬럼으로 Bounding Box 후보를 줄이고 `ST_Distance_Sphere`로 실제 반경을 검증해 ID만 조회·정렬·`LIMIT`
...
- `findByKeyword`: 제목·설명 FULLTEXT 검색, ID 조회 후 `findByIdxInWithOrganizer(ids)`로 주최자 페치
...
| `getNearbyMeetups()`          | 반경 기반 모임 조회   | `findNearbyMeetupIds` + `findByIdxInWithOrganizer`, `geo_point` Bounding Box + `ST_Distance_Sphere`, `maxResults` 클램프, @Timed                                     |
| `searchMeetupsByKeyword()`    | 키워드 검색           | `findByKeyword()` — 제목·설명 FULLTEXT 검색, ID 조회 후 주최자 페치                                     |
...
- AOP 기반 자동 성능 측정 (`@Timed` 어노테이션)
  ...
  - 주요 조회 메서드에 적용: `getAllMeetups()`, `getNearbyMeetups()`, `searchMeetupsByKeyword()`, `getAvailableMeetups()`
