...
| `updateMeetup()` | 모임 수정 | `findByIdWithDetails()`, MeetupNotFoundException, 선택적 업데이트 |
| `deleteMeetup()` | 모임 삭제 | MeetupNotFoundException, Soft Delete (isDeleted = true, deletedAt 설정) |
...
| `searchMeetupsByKeyword()` | 키워드 검색 | `findByKeyword()`, 제목/설명 검색 |
...
#### REST API
**참고**: 모든 엔드포인트는 클래스 레벨에 `@PreAuthorize("isAuthenticated()")` 적용되어 인증 필요. 인증 정보 없으면 `UnauthenticatedException`
...
| `/api/meetups/{meetupIdx}` | PUT | 모임 수정 (MeetupNotFoundException) | `{"meetup": {...}, "message": "..."}` |
| `/api/meetups/{meetupIdx}` | DELETE | 모임 삭제 (MeetupNotFoundException) | `{"message": "..."}` |
