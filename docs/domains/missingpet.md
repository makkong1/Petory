...
1. **DB에서 20km 반경 후보를 조회한다** — `ST_Within`(사각형, SPATIAL 인덱스)으로 좁힌 뒤
   `ST_Distance_Sphere <= 20km`로 정확한 원형까지 DB가 거른다.
2. 후보 수는 `max(200, size)`로 잡는다.
3. 각 후보에 대해 실종일 최신성과 거리 점수를 계산한다.
4. `score = 0.6 * recencyScore + 0.4 * distScore`로 정렬한다.
5. 부족하면 최신 실종 제보로 보충한다.

> **2026-07-31 지도 반경검색 통일**: 예전에는 DB가 **사각형 후보만** 주고 서비스가
> `haversineKm`으로 원형 반경을 걸렀다. 그러면 후보 상한(200건) 중 사각형 모서리에 걸린 것들이
> Java에서 버려져 **실제 점수 계산 대상이 200보다 적어졌다.** 반경 필터를 DB로 내려서
> 200건이 전부 반경 안이 됐고, care·meetup·location과 필터 위치가 같아졌다.
> `haversineKm`은 남아 있지만 이제 **거르는 용도가 아니라 점수(근접도)를 매기는 용도**다.
> 최종 정렬이 거리순이 아니라 점수순이라, 후보 선정 정렬은 `lostDate DESC`를 그대로 뒀다 —
> 네 도메인 중 여기만 정렬 기준이 다르다.
...
- 홈 추천의 **반경 필터는 DB**(`ST_Distance_Sphere`)가 하고, **점수 계산만 애플리케이션**에서 한다.
  점수식이 `0.6 * 최신성 + 0.4 * 근접도`라 SQL `ORDER BY`로 옮길 이유가 없다고 판단했다.
  다만 후보 상한(200건)이 곧 정확도 상한이라, 20km 안에 200건이 넘으면 그 밖은 점수 계산에서 빠진다.
