...
**"목격했어요" 버튼**:
- `POST /api/missing-pets/{boardIdx}/start-chat?witnessId={witnessId}` — 메서드에 `@PreAuthorize("isAuthenticated()")`
...
| `/api/missing-pets/{boardIdx}/start-chat` | POST | `witnessId` 필수 쿼리, `@PreAuthorize("isAuthenticated()")` |
