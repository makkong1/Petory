...
**댓글 작성**:
- 목격 위치(주소, 위도/경도) 저장
- 이미지 첨부 (FileTargetType.MISSING_PET_COMMENT)
- 게시글 작성자에게 알림 발송 (비동기, 작성자 ≠ 제보자일 때만)

### 2.3 채팅 연동

**"목격했어요" 버튼**:
- `POST /api/missing-pets/{boardIdx}/start-chat?witnessId={witnessId}` (인증 필요)
- 제보자(reporter)와 목격자(witness) 간 1:1 채팅방 생성
- `getUserIdByBoardIdx()`로 제보자 ID 경량 조회 (프로젝션 1쿼리)
...
| `BoardValidationException` | `statusRequired()`: status 파라미터 없음, `invalidStatus()`: 잘못된 status 값 |
...
#### MissingPetComment
- **BaseTimeEntity** 상속
- **필드**: idx, board, user, content, address, latitude, longitude, isDeleted, deletedAt
- **Soft Delete**: isDeleted, deletedAt
...
| 엔드포인트 | Method | 설명 |
|-----------|--------|------|
| `/api/missing-pets` | GET | 목록 조회 (페이징, status, page, size 기본값 0/20) |
| `/api/missing-pets/{id}` | GET | 상세 조회 (commentPage, commentSize, commentSize=0이면 댓글 제외) |
| `/api/missing-pets` | POST | 작성 (이메일 인증 필요) |
| `/api/missing-pets/{id}` | PUT | 수정 (이메일 인증 필요) |
| `/api/missing-pets/{id}/status` | PATCH | 상태 변경 (status: MISSING/FOUND/RESOLVED, BoardValidationException) |
| `/api/missing-pets/{id}` | DELETE | 삭제 (이메일 인증 필요, 관련 댓글 함께 Soft Delete) |
| `/api/missing-pets/{id}/comments` | GET | 댓글 목록 (페이징, page, size 기본값 0/20) |
| `/api/missing-pets/{id}/comments` | POST | 댓글 작성 |
| `/api/missing-pets/{boardId}/comments/{commentId}` | DELETE | 댓글 삭제 |
| `/api/missing-pets/{boardIdx}/start-chat` | POST | 채팅 시작 (인증 필요, witnessId 쿼리 파라미터) |

#### AdminMissingPetController (`/api/admin/missing-pets`, ADMIN/MASTER)
| 엔드포인트 | Method | 설명 |
|-----------|--------|------|
| `/api/admin/missing-pets/paging` | GET | 목록 조회 (status, deleted, q, page, size) |
| `/api/admin/missing-pets/{id}` | GET | 상세 조회 |
| `/api/admin/missing-pets/{id}/status` | PATCH | 상태 변경 |
| `/api/admin/missing-pets/{id}/delete` | POST | 삭제 |
| `/api/admin/missing-pets/{id}/restore` | POST | 복구 |
| `/api/admin/missing-pets/{boardId}/comments` | GET | 댓글 목록 (deleted 필터) |
| `/api/admin/missing-pets/{boardId}/comments/{commentId}/delete` | POST | 댓글 삭제 |
