...
- `JwtAuthenticationFilter`는 `CustomUserDetails.isEnabled()`와 `isAccountNonLocked()`를 확인한 뒤에만 `SecurityContext`에 인증 객체를 등록한다. 따라서 `BANNED` 또는 만료 전 `SUSPENDED` 사용자는 기존 access token이 남아 있어도 보호 API 인증이 거부된다.
...
- 관리자 상태 변경으로 `SUSPENDED` 또는 `BANNED`가 되면 refresh token을 제거한다.
...
- 관리자 상태 변경 API는 상태 필드와 refresh token 제거는 처리하지만, 아직 `UserSanctionService`를 경유하지 않아 `UserSanction` 이력을 항상 남기지는 않는다.
