...
**스케줄러 설정**:
- **실행 시간**: `application.properties`의 `statistics.scheduler.cron` (기본값: 매일 18:30)
- **집계 대상**: 어제 날짜의 통계
...
| 엔드포인트 | Method | 설명 | 권한 |
|-----------|--------|------|------|
| `/api/admin/statistics/daily` | GET | 기간별 일일 통계 조회 (startDate/endDate, 기본값: 최근 30일) | ADMIN, MASTER |
| `/api/admin/statistics/init` | POST | 과거 통계 초기화 (days 파라미터, 기본값: 30일) | MASTER |
| `/api/admin/statistics/scheduler/time` | GET | 스케줄러 실행 시간 조회 | ADMIN, MASTER |
| `/api/admin/statistics/scheduler/time` | PUT | 스케줄러 실행 시간 설정 (body: hour, minute) | MASTER |
