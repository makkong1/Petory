...
2. 반경 `radius`(m) 전달 — **`latitude`·`longitude`·`radius` 세 값이 모두 있어야** 컨트롤러가 위치 분기 선택
...
- **하이브리드 전략**: **`keyword`가 있으면** 키워드(FULLTEXT) 검색이 최우선 → 그다음 **`latitude`·`longitude`·`radius`가 모두 있으면** 위치 기반 → 아니면 지역 계층(`searchLocationServicesByRegion`, 파라미터 없으면 전체/평점순)
...
**컨트롤러 구현** (`LocationServiceController.searchLocationServices`):
- **우선순위**: `keyword` → (`latitude` **그리고** `longitude` **그리고** `radius`) → 지역 계층
- **위치 분기**: 세 값이 모두 있어야 함. `radius`만 빠지면 위치 검색이 아니라 지역 분기로 떨어짐(좌표만으로는 반경 검색 안 함)
- **기본값**: 위치 분기에서 `radius>0`이 아니면 **10000m**; `size`는 null이면 **100**, 0 이하이면 제한 없음(서비스에 null 전달)
...
**핵심 로직**:
- **반경 검색**: DB는 근사 영역 `ST_Within` + 거리 `ST_Distance_Sphere(...) <= radiusInMeters` (실제 쿼리는 `SpringDataJpaLocationServiceRepository.findByRadius` 참고)
- **POINT/공간**: `location` 컬럼(공간 타입)과 검색 중심 `POINT`를 사용
- **카테고리 필터링**: category3 → category2 → category1 순서로 검색 (대소문자 무시)
...
**Repository**: 메서드명은 `findByNameContaining` / `findByRadius` 등이며, 키워드 경로는 **FULLTEXT `MATCH ... AGAINST`** 를 사용합니다. 반경 경로는 코드상 **POLYGON 근사 + `ST_Distance_Sphere`** 조합입니다(위 스니펫은 구버전 예시로 삭제).
...
| `searchLocationServicesByLocation()` | 위치 기반 반경 검색 | `findByRadius`(ST_Within+ST_Distance_Sphere), 카테고리 필터링, Haversine 거리 DTO, 최대 결과 수 제한, 성능 로깅 |
| `searchLocationServicesByKeyword()` | 키워드 검색 | `findByNameContaining` → native `MATCH...AGAINST` (메서드명과 다름), 카테고리 필터링, 최대 결과 수 제한 |
...
| 엔드포인트 | Method | 설명 |
|-----------|--------|------|
| `/api/location-services/search` | GET | 하이브리드: `keyword` → (`latitude`+`longitude`+`radius`) → 지역. 위치 분기 시 `radius≤0`이면 10000m. `size` 기본 100, `size≤0`이면 전체. 응답 `{"services","count"}` |
...
**위치 기반 검색 요청 예시** (`radius` 필수):
```http
GET /api/location-services/search?latitude=37.5665&longitude=126.9780&radius=10000&category=동물약국&size=100
# radius 생략 시 위치 분기 아님 → 지역 분기(다른 파라미터 없으면 전체/평점순 등)
# size 생략 시 기본 100; 0 이하 → 전체
```

**잘못된 예** (좌표만 넣으면 반경 검색 안 함):
```http
# GET ...?latitude=37.5665&longitude=126.9780  → radius 없음 → 위치 반경 검색 아님
```
...
- **반경 단위**: 미터(m). 컨트롤러에서 `radius≤0`이면 10000m로 치환. **`radius` 없으면 위치 분기 자체가 실행되지 않음**
- **카테고리 필터링**: category3 → category2 → category1 순서로 검색 (대소문자 무시)
- **성능 측정**: 각 단계별 실행 시간 로깅 (DB 쿼리 시간, 필터링 시간, DTO 변환 시간)
- **하이브리드 전략**: **`keyword`가 있으면** 키워드(FULLTEXT) 검색이 최우선 → 그다음 **`latitude`·`longitude`·`radius`가 모두 있으면** 위치 기반 → 아니면 지역 계층(`searchLocationServicesByRegion`, 파라미터 없으면 전체/평점순)

### 8.3 하이브리드 데이터 로딩 전략
- **초기 로드**: 클라이언트가 반경(예: 5km)을 넘기면 위치 분기 — 백엔드 단독 기본값은 아님
- **✅ 일관성 개선 (2026-02-04)**: 지역 선택 시 항상 백엔드 재요청하여 검색 결과 일관성 확보
