...
- **UX 설계 원칙**:
  ...
  - **검색창 주소 geocoding**: 검색어 입력 시 Naver Geocoding API로 좌표 변환 먼저 시도. 성공하면 지도 중심 이동 + keyword 초기화 후 반경 검색. 실패하면 시설명 keyword 검색 fallback. 읍면동(예: "묵동")·시군구(예: "강남구")·도로명 등 주소 계층 전체 지원.
