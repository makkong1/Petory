...
#### 5. 펫코인 에스크로 동시성 문제
**시나리오**: 거래 확정·완료·취소 시 동시 요청

**문제**: 잔액 차감, 에스크로 상태 변경, 지급/환불 시 Race Condition

**위치**: `domain/payment/service/PetCoinEscrowService.java`, `PetCoinService.java`
...
#### 3. 프로젝트에서 사용한 방법
**DB 제약조건 + 원자적 쿼리 + 비관적 락**:
- UNIQUE 제약조건: 중복 방지 (소셜 로그인 등)
- 원자적 증가 쿼리: Lost Update 해결 (경고 횟수, 모임 참여 인원)
- 비관적 락: 펫코인 에스크로 (`findByIdForUpdate`, `findByCareRequestForUpdate`)
...
#### 2. 해결 방법
**위치**: `domain/meetup/service/MeetupService.java`, `SpringDataJpaMeetupRepository.incrementParticipantsIfAvailable()`
...
@Transactional 트랜잭션 시작
  ↓
원자적 UPDATE: currentParticipants + 1 (WHERE currentParticipants < maxParticipants)
  ↓
영향받은 행 0이면 인원 초과 → 예외
  ↓
MeetupParticipants 저장
```
...
// MeetupService.joinMeetup() - 원자적 증가 후 영향받은 행 확인
int updated = meetupRepository.incrementParticipantsIfAvailable(meetupIdx);
if (updated == 0) {
    throw MeetupConflictException.participantLimitReached();
}
```
...
### 모임 참여 동시성 해결
- **원자적 UPDATE**: `incrementParticipantsIfAvailable()` - WHERE 조건으로 인원 초과 시 0행 반환
- **효과**: Race Condition 방지, 최대 인원 초과 방지

### 펫코인 에스크로 동시성
- **비관적 락**: `@Lock(PESSIMISTIC_WRITE)` - 잔액 차감, 에스크로 상태 변경 시
- **효과**: 중복 지급/환불 방지
