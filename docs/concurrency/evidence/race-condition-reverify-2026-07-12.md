...
> ⚠️ **보강 (2026-07-30) — 위 해석은 메커니즘의 절반만 짚었다.** `a549eb33`의 `joinMeetup` 본문을 직접 열어보면 순서가 이렇다:
>
> ```java
> MeetupParticipants savedParticipant = meetupParticipantsRepository.save(participant); // ① FK(meetup_idx) → meetup 행 S락
> if (!meetup.getOrganizer().getIdx().equals(userIdx)) {
>     meetup.setCurrentParticipants(meetup.getCurrentParticipants() + 1);
>     meetupRepository.save(meetup);                                                     // ② 같은 행 X락 요구
> }
> ```
>
> 즉 단순히 "여러 스레드가 UPDATE에서 경합"한 게 아니라, **참가자 INSERT가 FK로 meetup 행에 S락을 먼저 잡은 상태에서 ②가 같은 행의 X락을 요구하는 락 승격**이다. 동시 요청 전원이 S락을 쥔 채 X락을 노리므로 순환 대기가 구조적으로 생긴다. → **경고 카운트 데드락(`1f989b9f`)과 문자 그대로 동일한 패턴**이며, 두 사례의 해법(X락을 먼저 확보)도 같은 원리다. 이 공통점은 2026-07-30 재점검에서야 확인됐다.

**정리**: Meetup의 동시성 결함은 **네 단계**로 존재했다. *(2026-07-30 정정: 아래 4단계를 추가. 이 문서는 원래 `bf32d155`를 "현재"로 적었는데, 실제로는 그 뒤에 락이 재도입돼 있었다.)*
1. **`a549eb33`(2025-12-13, 락 없음)**: 사용자 경험 관점의 결함 — 정당한 참가 요청이 데드락으로 실패(인원초과는 아님)
2. **`a5943b18`(2025-12-19, Pessimistic Lock)**: 데드락도 인원초과도 없이 안전, 다만 락 대기로 인한 처리량 저하 가능성
3. **`bf32d155`(2025-12-20, 락 제거 + 원자적 UPDATE)**: 락 경합 없이 안전 + 처리량 유지. **부수 효과**: 인원 UPDATE가 참가자 INSERT **앞으로** 이동해 위 락 승격 구조도 함께 사라졌다(당시엔 인지하지 못함).
4. **`58467b4e`(2026-05-09, 락 재도입, 현재)**: `findByIdWithLock`을 다시 넣고 원자적 UPDATE를 **유지** + `entityManager.refresh()`. 현재 코드는 **락 + 조건부 UPDATE + 복합 PK + CHECK의 계층 방어**다. → "Meetup은 락이 아니라 원자적 UPDATE로 해결"이라는 서술은 3단계 시점엔 맞았고 **4단계 이후 낡았다.**

"인원초과 버그"는 **정상 서비스 호출 경로에서는** 재현되지 않고(데드락 방어선에 먼저 막힘), 최초 버그의 관찰 가능한 증상은 **부당한 요청 실패(Deadlock)** 였다는 게 이번 재검증의 핵심 발견이다.

> ⚠️ **보강 (2026-07-30)**: 위 문장을 원래 "인원초과는 트랜잭션을 우회한 별도 테스트에서만 확인 가능한 **이론적 위험**"이라고 적었으나, `testRaceConditionWithoutTransaction`을 재실행하니 **실제로 재현된다**: 3명 전원 성공, **DB 참가자 4명 vs `currentParticipants` 2, 정원 3** — 인원 초과와 카운터 Lost Update가 동시에 발생. 따라서 "이론적 위험"은 **과소 표현**이고, 정확히는 **"정상 트랜잭션 경계에서는 데드락에 먼저 막혀 도달하지 못하지만, 경계를 우회하면 결정론적으로 재현되는 결함"**이다.
