# 커뮤니티 & 실종 제보 아키텍처

## 📋 개요

커뮤니티 & 실종 제보 도메인은 사용자들이 게시글을 작성하고 공유하며, 실종 동물 정보를 지도 기반으로 공유할 수 있는 핵심 도메인입니다. Magazine + Smart Grid 레이아웃, 인기 게시글 추적, 블라인드 처리, 실종 골든타임 위치 공유 등의 기능을 제공합니다.

## 🏗️ 시스템 아키텍처

### 전체 구조도

```mermaid
graph TB
    subgraph "Frontend"
        FE[React Frontend<br/>CommunityBoard<br/>MissingPetBoardPage]
    end
    
    subgraph "Backend API Layer"
        BC[BoardController<br/>/api/boards]
        MPC[MissingPetBoardController<br/>/api/missing-pets]
        ABC[AdminBoardController<br/>/api/admin/boards]
        RC[ReportController<br/>/api/reports]
    end
    
    subgraph "Service Layer"
        BS[BoardService<br/>게시글 CRUD, 배치 조회]
        MPS[MissingPetBoardService<br/>실종 제보 관리]
        BPS[BoardPopularityService<br/>인기 게시글 추적]
        BPSCH[BoardPopularityScheduler<br/>스냅샷 자동 생성]
        RS[ReportService<br/>신고 처리]
        CS[CommentService<br/>댓글 관리]
    end
    
    subgraph "Data Layer"
        BR[BoardRepository]
        MPR[MissingPetBoardRepository]
        CR[CommentRepository]
        BPSR[BoardPopularitySnapshotRepository]
        BRR[BoardReactionRepository]
        BVR[BoardViewLogRepository]
        RR[ReportRepository]
        MySQL[(MySQL<br/>Board<br/>MissingPetBoard<br/>BoardPopularitySnapshot<br/>Report)]
    end
    
    subgraph "External Integration"
        FILE[File Domain<br/>파일 첨부]
        NOTI[Notification Domain<br/>알림 발송]
        MAP[Location Domain<br/>지도 표시]
    end
    
    FE -->|게시글 조회/작성| BC
    FE -->|실종 제보 조회/작성| MPC
    FE -->|신고| RC
    
    BC --> BS
    MPC --> MPS
    ABC --> BS
    RC --> RS
    
    BS --> BR
    BS --> BRR
    BS --> BVR
    MPS --> MPR
    BPS --> BPSR
    BPS --> BRR
    BPS --> CR
    BPS --> BVR
    
    BPSCH --> BPS
    
    RS --> RR
    RS --> BR
    RS --> CR
    
    BR --> MySQL
    MPR --> MySQL
    BPSR --> MySQL
    RR --> MySQL
    
    BS --> FILE
    MPS --> FILE
    CS --> NOTI
    MPS --> NOTI
    
    style BS fill:#e1f5ff
    style BPS fill:#fff4e1
    style MPS fill:#ffe1f5
    style RS fill:#e1ffe1
```

## 🔧 핵심 컴포넌트

### 1. BoardService (커뮤니티 게시판 관리)

**역할**: 게시글 CRUD, 배치 조회로 N+1 문제 해결, 블라인드 처리

**주요 메서드**:
- `getAllBoardsWithPaging()`: 게시글 목록 조회 (카테고리·페이징)
- `mapBoardsWithReactionsBatch()`: 배치 조회로 N+1 문제 해결
- `updateBoardStatus()`: 게시글 상태 변경 (블라인드 처리)

**핵심 로직**:

#### 배치 조회로 N+1 문제 해결
```java
private List<BoardDTO> mapBoardsWithReactionsBatch(List<Board> boards) {
    // 1. 게시글 ID 목록 추출
    List<Long> boardIds = boards.stream()
        .map(Board::getIdx)
        .collect(Collectors.toList());
    
    // 2. 좋아요/싫어요 카운트 배치 조회 (500개 단위)
    Map<Long, Map<ReactionType, Long>> reactionCountsMap = 
        getReactionCountsBatch(boardIds);
    
    // 3. 첨부파일 배치 조회
    Map<Long, List<FileDTO>> attachmentsMap = 
        attachmentFileService.getAttachmentsBatch(FileTargetType.BOARD, boardIds);
    
    // 4. DTO 변환 및 반응 정보 매핑
    return boards.stream()
        .map(board -> {
            BoardDTO dto = boardConverter.toDTO(board);
            Map<ReactionType, Long> counts = reactionCountsMap.getOrDefault(
                board.getIdx(), new HashMap<>());
            dto.setLikes(Math.toIntExact(counts.getOrDefault(ReactionType.LIKE, 0L)));
            dto.setDislikes(Math.toIntExact(counts.getOrDefault(ReactionType.DISLIKE, 0L)));
            dto.setAttachments(attachmentsMap.getOrDefault(board.getIdx(), new ArrayList<>()));
            return dto;
        })
        .collect(Collectors.toList());
}
```

#### 블라인드 처리
- 관리자가 수동으로 블라인드 처리 (`ContentStatus.BLINDED`)
- 블라인드된 게시글은 내용이 가려짐
- **참고**: 신고 누적 시 자동 블라인드 처리는 현재 구현되지 않음 (관리자 수동 처리만 가능)

### 2. BoardPopularityService (인기 게시글 추적)

**역할**: 조회수, 반응 수 기반 인기 게시글 스냅샷 생성

**주요 메서드**:
- `generateSnapshots()`: 인기 게시글 스냅샷 생성
- `getPopularBoards()`: 인기 게시글 조회 (다단계 전략)
- `calculatePopularityScore()`: 인기도 점수 계산

**핵심 로직**:

#### 인기도 점수 계산
```java
private int calculatePopularityScore(int likes, int comments, int views) {
    return (likes * 3) + (comments * 2) + views;
}
```

**점수 공식**:
- 좋아요: 3점
- 댓글: 2점
- 조회수: 1점

#### 스냅샷 생성 프로세스
1. "자랑" 카테고리 게시글 조회 (기간 내)
2. 배치 조회로 좋아요/댓글/조회수 집계 (500개 단위)
3. 인기도 점수 계산 및 정렬
4. 상위 30개 게시글 스냅샷 저장
5. 기존 스냅샷 삭제 후 새로 생성

#### 다단계 조회 전략
```java
// 1. 정확한 날짜 매칭으로 조회 시도
List<BoardPopularitySnapshot> snapshots = snapshotRepository
    .findByPeriodTypeAndPeriodStartDateAndPeriodEndDateOrderByRankingAsc(...);

// 2. 정확한 매칭이 없으면 기간이 겹치는 스냅샷 조회
if (snapshots.isEmpty()) {
    snapshots = snapshotRepository
        .findByPeriodTypeAndPeriodStartDateLessThanEqualAndPeriodEndDateGreaterThanEqualOrderByRankingAsc(...);
}

// 3. 그래도 없으면 가장 최근 스냅샷 조회
if (snapshots.isEmpty()) {
    snapshots = snapshotRepository
        .findTop30ByPeriodTypeOrderByPeriodEndDateDescRankingAsc(periodType);
}

// 4. 모든 시도가 실패하면 새로 생성
if (snapshots.isEmpty()) {
    snapshots = generateSnapshots(periodType, range);
}
```

### 3. BoardPopularityScheduler (스냅샷 자동 생성)

**역할**: 주기적으로 인기 게시글 스냅샷 자동 생성

**스케줄러 설정**:
- 주간 스냅샷: 매일 오후 6시 30분 (`@Scheduled(cron = "0 30 18 * * ?")`)
- 월간 스냅샷: 매주 월요일 오후 6시 30분 (`@Scheduled(cron = "0 30 18 ? * MON")`)

**기간 계산**:
- 주간: 오늘 포함 최근 7일 (today - 6 days ~ today)
- 월간: 오늘 포함 최근 30일 (today - 29 days ~ today)

### 4. MissingPetBoardService (실종 제보 관리)

**역할**: 실종 동물 신고 관리, 위치 기반 검색

**주요 메서드**:
- `createBoard()`: 실종 제보 생성 (이메일 인증 필수)
- `getBoardsWithPaging()`: 실종 제보 목록 조회 (상태·페이징)
- 위도/경도 저장 및 상세·목록 응답에 포함 (지도 마커 등). **서버 측 반경 검색 전용 API는 없을 수 있음** — 클라이언트/추후 확장에 따름.

**핵심 로직**:

#### 실종 위치 공유
- 위도/경도 저장 (`BigDecimal` 타입, precision=15, scale=12)
- 지도 기반 직관적인 위치 표시
- 목록·필터는 주로 `status` 등; 반경 검색은 설계상 언급 가능하나 **백엔드 구현 범위는 코드 기준으로 확인**

#### 상태 관리
- `MISSING`: 실종 중 (기본 상태)
- `FOUND`: 발견됨
- `RESOLVED`: 해결됨

### 5. ReportService (신고 처리)

**역할**: 신고 접수 및 처리, 관리자 수동 블라인드 처리

**주요 기능**:
- 신고 접수 (게시글, 댓글, 실종제보 등)
- 중복 신고 방지 (Unique 제약조건)
- 관리자 수동 블라인드 처리
- **참고**: 신고 누적 시 자동 블라인드 처리는 현재 구현되지 않음

## 🔄 비즈니스 로직 흐름

### 1. 게시글 작성 흐름

**단계별 처리 과정:**

1. **사용자 요청**
   - 프론트엔드에서 게시글 작성 폼 제출
   - `POST /api/boards` 요청 (로그인 필수: `@PreAuthorize("isAuthenticated()")`)

2. **서비스 레이어 처리** (`BoardService.createBoard()`)
   - 사용자 정보 조회 및 검증
   - 게시글 엔티티 생성 (제목, 내용, 카테고리, 작성자)
   - DB 저장 (`boardRepository.save()`)

3. **파일 첨부 처리**
   - 첨부파일이 있는 경우 `AttachmentFileService.syncSingleAttachment()` 호출
   - `FileTargetType.BOARD`로 파일 정보 저장 및 연결

4. **캐시 무효화**
   - 게시글 목록 캐시 전체 무효화 (`@CacheEvict(value = "boardList", allEntries = true)`)
   - 새로운 게시글이 추가되므로 목록 캐시 갱신 필요

5. **응답 반환**
   - 저장된 게시글 정보를 DTO로 변환하여 반환
   - 반응 정보(좋아요/싫어요), 첨부파일 정보 포함

**특징:**
- 게시글 작성은 이메일 인증 불필요 (조회/작성은 1단계 권한)
- 수정/삭제 시에만 이메일 인증 필요 (2단계 권한)

### 2. 게시글 수정/삭제 흐름

**수정 흐름** (`BoardService.updateBoard()`):

1. **권한 확인**
   - 게시글 존재 여부 확인
   - **이메일 인증 필수**: `user.getEmailVerified()` 확인
   - 미인증 시 `EmailVerificationRequiredException` 발생

2. **데이터 업데이트**
   - 제목, 내용, 카테고리 업데이트 (null이 아닌 값만)
   - 첨부파일이 있으면 파일 동기화

3. **캐시 무효화**
   - 게시글 상세 캐시 무효화 (`@CacheEvict(value = "boardDetail", key = "#idx")`)
   - 게시글 목록 캐시 무효화

**삭제 흐름** (`BoardService.deleteBoard()`):

1. **권한 확인**
   - 게시글 존재 여부 확인
   - **이메일 인증 필수**: `user.getEmailVerified()` 확인

2. **Soft Delete 처리**
   - `isDeleted = true` 설정
   - `status = ContentStatus.DELETED` 설정
   - `deletedAt = LocalDateTime.now()` 기록

3. **연관 데이터 처리**
   - 해당 게시글의 모든 댓글도 일괄 삭제 처리
   - 댓글도 `isDeleted = true`, `status = DELETED` 설정
   - 트랜잭션으로 원자성 보장

4. **캐시 무효화**
   - 게시글 상세 및 목록 캐시 무효화

**특징:**
- Soft Delete 방식으로 데이터 보존 (복구 가능)
- 게시글 삭제 시 댓글도 함께 삭제 처리

### 3. 댓글 작성 흐름

**단계별 처리 과정** (`CommentService.addComment()`):

1. **데이터 검증**
   - 게시글 존재 여부 확인
   - 사용자 정보 확인

2. **댓글 저장**
   - 댓글 엔티티 생성 및 저장
   - 첨부파일이 있으면 파일 동기화

3. **게시글 댓글 수 실시간 업데이트**
   - `board.commentCount` 증가 (`incrementBoardCommentCount()`)
   - 게시글 저장하여 카운트 반영

4. **알림 발송**
   - 댓글 작성자가 게시글 작성자와 다른 경우에만 알림 발송
   - `NotificationService.createNotification()` 호출
   - 알림 타입: `NotificationType.BOARD_COMMENT`
   - 알림 내용: "{작성자명}님이 댓글을 남겼습니다: {댓글 내용(50자 제한)}"

5. **캐시 무효화**
   - 게시글 상세 캐시 무효화 (댓글 수 변경 반영)

**댓글 삭제 흐름** (`CommentService.deleteComment()`):

1. **권한 확인**
   - 댓글 존재 여부 및 게시글 소속 확인
   - **이메일 인증 필수**: `user.getEmailVerified()` 확인

2. **Soft Delete 처리**
   - `isDeleted = true`, `status = DELETED` 설정

3. **게시글 댓글 수 실시간 업데이트**
   - `board.commentCount` 감소 (`decrementBoardCommentCount()`)
   - 최소값 0 보장 (`Math.max(0, currentCount - 1)`)

**특징:**
- 댓글 작성은 이메일 인증 불필요
- 댓글 삭제는 이메일 인증 필수 (`deleteComment`). `CommentService.updateComment`(수정)는 이메일 인증을 요구하지만, **일반 사용자용 REST는 `BoardController`에 없을 수 있음**(관리자·내부 호출 위주).
- 게시글 댓글 수는 실시간으로 업데이트 (인기 게시글 점수 계산에 영향)

### 4. 게시글 조회 및 조회수 증가 흐름

**단계별 처리 과정** (`BoardService.getBoard()`):

1. **게시글 조회**
   - 게시글 ID로 조회
   - 캐시 확인 (`@Cacheable(value = "boardDetail", key = "#idx")`)

2. **조회수 증가 조건 확인** (`shouldIncrementView()`)
   - 로그인하지 않은 사용자: 항상 증가
   - 로그인한 사용자: `BoardViewLog` 확인
     - 해당 사용자가 이미 조회한 기록이 있으면 증가하지 않음
     - 기록이 없으면 `BoardViewLog` 생성 후 조회수 증가

3. **조회수 증가** (`incrementViewCount()`)
   - `board.viewCount` 1 증가
   - DB 저장

4. **상세 정보 매핑**
   - 반응 정보(좋아요/싫어요), 첨부파일 정보 포함하여 DTO 변환

**특징:**
- 사용자별 1회 조회 제한으로 중복 조회수 증가 방지
- 조회수는 인기 게시글 점수 계산에 포함 (1점)

### 5. 실종 제보 작성 흐름

**단계별 처리 과정** (`MissingPetBoardService.createBoard()`):

1. **이메일 인증 확인**
   - **이메일 인증 필수**: `user.getEmailVerified()` 확인
   - 미인증 시 `EmailVerificationRequiredException` 발생
   - 실종 제보는 책임 있는 행동이므로 2단계 권한 필요

2. **위치 정보 처리**
   - 프론트엔드에서 주소를 좌표로 변환 (Geocoding API)
   - 위도(`latitude`), 경도(`longitude`)를 `BigDecimal` 타입으로 저장
   - 정밀도: precision=15, scale=12

3. **실종 제보 저장**
   - 반려동물 정보 (이름, 종류, 품종, 성별, 나이, 색상)
   - 실종 정보 (실종일, 실종 위치, 위도/경도)
   - 상태: 기본값 `MISSING`

4. **파일 첨부 처리**
   - 반려동물 사진 첨부 시 `FileTargetType.MISSING_PET`로 저장

5. **응답 반환**
   - 저장된 실종 제보 정보를 DTO로 변환하여 반환
   - 지도에 마커 표시를 위한 위치 정보 포함

**실종 제보 댓글 작성** (`MissingPetBoardService.addComment()`):

1. **목격 정보 포함**
   - 댓글에 목격 위치 정보 포함 가능 (주소, 위도/경도)
   - 목격 사진 첨부 가능 (`FileTargetType.MISSING_PET_COMMENT`)

2. **알림 발송**
   - 댓글 작성자가 제보자와 다른 경우 알림 발송 (비동기 처리)
   - 알림 타입: `NotificationType.MISSING_PET_COMMENT`
   - 알림 실패 시에도 댓글 작성은 성공하도록 분리

3. **채팅 연동**
   - "목격했어요" 버튼 클릭 시 1:1 채팅방 자동 생성
   - 제보자와 목격자 간 소통 가능

**상태 변경** (`MissingPetBoardService.updateStatus()`):
- `MISSING` → `FOUND` (발견됨)
- `MISSING` → `RESOLVED` (해결됨)
- 제보자 또는 관리자가 상태 변경 가능

### 6. 블라인드 처리 흐름

**관리자 수동 블라인드 처리** (`AdminBoardController`):

1. **권한 확인**
   - 관리자 권한 확인 (`@PreAuthorize("hasAnyRole('ADMIN','MASTER')")`)

2. **상태 변경**
   - 게시글/댓글 상태를 `ContentStatus.BLINDED`로 변경
   - `BoardService.updateBoardStatus()` 또는 `CommentService.updateCommentStatus()` 호출

3. **캐시 무효화**
   - 게시글 상세 및 목록 캐시 무효화

4. **프론트엔드 처리**
   - 블라인드된 콘텐츠는 내용이 가려짐
   - "관리자에 의해 차단된 콘텐츠입니다" 메시지 표시

**현재 구현 상태:**
- 관리자 수동 블라인드 처리: ✅ 구현됨
- 신고 누적 시 자동 블라인드 처리: ❌ 구현되지 않음 (추후 구현 가능)

### 7. 반응(좋아요/싫어요) 처리 흐름

**게시글 반응** (`ReactionService.reactToBoard()`):

1. **중복 확인**
   - DB Unique 제약조건으로 동일 사용자의 중복 반응 방지
   - 기존 반응이 있으면 반응 타입 변경 (좋아요 ↔ 싫어요)

2. **반응 저장/업데이트**
   - 새로운 반응이면 저장
   - 기존 반응이면 반응 타입 업데이트

3. **반응 요약 반환**
   - 좋아요 수, 싫어요 수 집계하여 반환
   - 프론트엔드에서 실시간 업데이트

**특징:**
- 좋아요는 인기 게시글 점수에 3점 반영
- 싫어요는 점수에 반영되지 않음

## 📊 데이터 흐름

### 1. 인기 게시글 스냅샷 생성 흐름

```mermaid
sequenceDiagram
    participant Scheduler as BoardPopularityScheduler
    participant BPS as BoardPopularityService
    participant BR as BoardRepository
    participant BRR as BoardReactionRepository
    participant CR as CommentRepository
    participant BVR as BoardViewLogRepository
    participant BPSR as BoardPopularitySnapshotRepository
    participant MySQL
    
    Note over Scheduler: 매일 18:30 (주간)<br/>매주 월요일 18:30 (월간)
    Scheduler->>BPS: generateSnapshots(periodType)
    
    BPS->>BPS: 기간 계산 (주간: 7일, 월간: 30일)
    BPS->>BR: findByCategoryAndCreatedAtBetween("자랑", start, end)
    BR->>MySQL: SELECT * FROM board WHERE category='자랑'
    MySQL-->>BR: 게시글 목록
    BR-->>BPS: List<Board>
    
    BPS->>BPS: boardIds 추출
    
    loop 배치 단위로 (500개씩)
        BPS->>BRR: countByBoardsGroupByReactionType(batch)
        BRR->>MySQL: 좋아요 카운트 집계
        MySQL-->>BRR: 좋아요 카운트
        
        BPS->>CR: countByBoardsAndIsDeletedFalse(batch)
        CR->>MySQL: 댓글 카운트 집계
        MySQL-->>CR: 댓글 카운트
        
        BPS->>BVR: countByBoards(batch)
        BVR->>MySQL: 조회수 카운트 집계
        MySQL-->>BVR: 조회수 카운트
    end
    
    BPS->>BPS: 인기도 점수 계산<br/>(likes*3 + comments*2 + views)
    BPS->>BPS: 점수순 정렬, 상위 30개 선택
    
    BPS->>BPSR: deleteByPeriodTypeAndPeriodStartDateAndPeriodEndDate()
    BPSR->>MySQL: DELETE 기존 스냅샷
    MySQL-->>BPSR: 삭제 완료
    
    BPS->>BPSR: saveAll(snapshots)
    BPSR->>MySQL: INSERT 스냅샷 (30개)
    MySQL-->>BPSR: 저장 완료
    BPSR-->>BPS: List<BoardPopularitySnapshot>
```

### 2. Magazine + Smart Grid 레이아웃 흐름

```mermaid
sequenceDiagram
    participant User as 사용자
    participant Frontend
    participant BoardController
    participant BoardService
    participant MySQL
    
    User->>Frontend: 커뮤니티 페이지 접속
    Frontend->>BoardController: GET /api/boards?category={category}
    BoardController->>BoardService: getAllBoardsWithPaging(category, page, size)
    
    BoardService->>MySQL: 페이징·카테고리 조건 조회
    MySQL-->>BoardService: 게시글 목록
    
    BoardService->>BoardService: 배치 조회 (반응, 첨부파일)
    BoardService-->>Frontend: List<BoardDTO>
    
    Frontend->>Frontend: 게시글 분류 (categorizedPosts)
    
    Note over Frontend: large: 공지사항 또는<br/>첫 번째 이미지 게시글
    Note over Frontend: medium: 이미지 있는 게시글
    Note over Frontend: small: 텍스트만 게시글
    
    Frontend->>Frontend: Smart Grid 레이아웃 적용
    Note over Frontend: large: grid-column span 12<br/>medium: grid-column span 6<br/>small: grid-column span 3
    
    Frontend-->>User: Magazine 스타일 게시글 목록 표시
```

### 3. 실종 골든타임 위치 공유 흐름

```mermaid
sequenceDiagram
    participant User as 사용자
    participant Frontend
    participant MissingPetBoardController
    participant MissingPetBoardService
    participant MapService as Location Domain
    participant MySQL
    
    User->>Frontend: 실종 제보 작성
    Frontend->>MapService: 주소 → 좌표 변환 (Geocoding)
    MapService-->>Frontend: {latitude, longitude}
    
    Frontend->>MissingPetBoardController: POST /api/missing-pets
    MissingPetBoardController->>MissingPetBoardService: createBoard(dto)
    
    MissingPetBoardService->>MissingPetBoardService: 이메일 인증 확인
    
    MissingPetBoardService->>MySQL: INSERT MissingPetBoard<br/>(위도/경도 포함)
    MySQL-->>MissingPetBoardService: 저장 완료
    
    MissingPetBoardService->>MissingPetBoardService: 파일 첨부 처리
    MissingPetBoardService-->>Frontend: MissingPetBoardDTO
    
    Frontend->>Frontend: 지도에 마커 표시<br/>(위도/경도 기반)
    Frontend-->>User: 실종 위치 지도 표시
```

## 🎯 핵심 설계 전략

### 1. Magazine + Smart Grid 레이아웃 전략

**문제**: 다양한 크기의 게시글을 시각적으로 구분하여 표시해야 함

**해결**: 크기별 게시글 분류 및 그리드 레이아웃
- **대형 카드 (Large)**: 공지사항 또는 첫 번째 이미지 게시글 (grid-column: span 12)
- **중형 카드 (Medium)**: 이미지 있는 게시글 (grid-column: span 6)
- **소형 카드 (Small)**: 텍스트만 게시글 (grid-column: span 3)

**프론트엔드 구현**:
```javascript
const categorizedPosts = useMemo(() => {
    const large = [];
    const medium = [];
    const small = [];
    
    // 공지사항 또는 첫 번째 이미지 게시글을 대형으로
    const noticePost = filteredPosts.find(post => post.category === '공지');
    const firstWithImage = filteredPosts.find(post => post.boardFilePath);
    
    if (noticePost) {
        large.push(noticePost);
    } else if (firstWithImage) {
        large.push(firstWithImage);
    }
    
    // 나머지 게시글 분류
    filteredPosts.forEach((post) => {
        if (large.includes(post)) return;
        if (post.boardFilePath) {
            medium.push(post);
        } else {
            small.push(post);
        }
    });
    
    return { large, medium, small };
}, [filteredPosts]);
```

**효과**:
- 시각적으로 매력적인 레이아웃
- 공지사항 및 이미지 게시글 강조
- 반응형 디자인 지원 (PC/Tablet/Mobile)

### 2. 인기 게시글 추적 전략

**문제**: 실시간으로 인기 게시글을 계산하면 성능 부하가 큼

**해결**: 스냅샷 기반 인기 게시글 관리
- 주기적으로 스냅샷 생성 (주간/월간)
- 인기도 점수 = (좋아요 × 3) + (댓글 × 2) + 조회수
- "자랑" 카테고리 게시글만 대상
- 상위 30개 게시글만 스냅샷 저장

**다단계 조회 전략**:
1. 정확한 날짜 매칭으로 조회
2. 기간이 겹치는 스냅샷 조회
3. 가장 최근 스냅샷 조회
4. 없으면 새로 생성

**효과**:
- 실시간 계산 부하 감소
- 빠른 인기 게시글 조회
- 일관된 인기도 점수 제공

### 3. 배치 조회 전략 (N+1 문제 해결)

**문제**: 게시글 목록 조회 시 반응, 첨부파일 조회로 N+1 문제 발생

**해결**: 배치 조회로 한 번에 조회
- 게시글 ID 목록 추출
- 500개 단위로 배치 분할
- 배치 단위로 반응/첨부파일 조회
- Map으로 결과 매핑

**효과**:
- N+1 문제 해결
- 쿼리 수 대폭 감소
- 응답 시간 단축

### 4. 블라인드 처리 전략

**문제**: 유해 콘텐츠를 사용자에게 노출하지 않아야 함

**해결**: ContentStatus 기반 블라인드 처리
- `ContentStatus.BLINDED` 상태로 게시글/댓글 가리기
- 관리자가 수동으로 블라인드 처리
- 블라인드된 콘텐츠는 내용이 가려짐

**현재 구현 상태**:
- 관리자 수동 블라인드 처리: 구현됨
- 신고 누적 시 자동 블라인드 처리: 구현되지 않음 (추후 구현 가능)

**효과**:
- 유해 콘텐츠 차단
- 관리자 제어 가능

### 5. 실종 골든타임 위치 공유 전략

**문제**: 실종 동물의 위치를 직관적으로 공유해야 함

**해결**: 지도 기반 위치 공유
- 위도/경도 저장 (`BigDecimal`, precision=15, scale=12)
- 지도에 마커 표시
- 목격 정보 댓글에도 위치 정보 포함 (`MissingPetComment` 등)

**효과**:
- 직관적인 위치 공유
- 골든타임 활용 극대화
- 반경·근처 탐색은 주로 **클라이언트/지도**에서 처리(백엔드 전용 반경 검색 API는 선택 사항)

## 🔄 도메인 간 연동

### 1. File 도메인 연동
- **용도**: 게시글 및 실종 제보 이미지 첨부
- **방법**: `AttachmentFileService.syncSingleAttachment()` 호출
- **효과**: 파일 관리 일원화

### 2. Notification 도메인 연동
- **용도**: 댓글 작성 시 게시글 작성자에게 알림
- **방법**: `NotificationService.createNotification()` 호출
- **효과**: 실시간 알림으로 사용자 참여도 향상

### 3. Location 도메인 연동
- **용도**: 실종 제보 위치 표시 및 검색
- **방법**: 위도/경도 저장 및 지도 표시
- **효과**: 직관적인 위치 공유

### 4. Report 도메인 연동
- **용도**: 게시글/댓글 신고 및 블라인드 처리
- **방법**: `ReportService`를 통한 신고 처리
- **효과**: 유해 콘텐츠 관리

## 📈 성능 최적화

### 1. DB 최적화

#### 인덱스 전략
```sql
-- 카테고리별 게시글 조회
CREATE INDEX idx_board_category_created ON board(category, created_at DESC);

-- 삭제 여부 인덱스
CREATE INDEX idx_board_deleted ON board(is_deleted);

-- 상태별 조회
CREATE INDEX idx_board_status ON board(status);

-- 인기 게시글 스냅샷 조회
CREATE INDEX idx_snapshot_period ON board_popularity_snapshot(period_type, period_start_date, period_end_date, ranking);

-- 실종 제보 위치 기반 검색
CREATE INDEX idx_missing_pet_location ON MissingPetBoard(latitude, longitude);

-- 실종 제보 상태별 조회
CREATE INDEX idx_missing_pet_status ON MissingPetBoard(status);
```

**선정 이유**:
- 카테고리별 필터링이 빈번함
- 인기 게시글 스냅샷 조회 최적화
- 실종 제보 위치 기반 검색 최적화

### 2. 애플리케이션 레벨 최적화

#### 배치 조회 최적화
- **500개 단위 배치 분할**: IN 절 크기 제한 회피
- **Map 기반 매핑**: O(1) 조회 성능
- **스트림 처리**: Java Stream API로 효율적인 처리

#### 스냅샷 기반 인기 게시글
- **주기적 생성**: 실시간 계산 부하 감소
- **다단계 조회 전략**: 캐시 히트율 향상
- **상위 30개만 저장**: 저장 공간 최적화

#### 캐싱 전략
- 인기 게시글 스냅샷: 다단계 조회 전략으로 캐시 효과
- 게시글 목록: 캐시 임시 비활성화 (개발 중 데이터 동기화 문제)

## 🔐 보안 고려사항

### 1. 권한 제어
- 게시글 **작성**: 로그인만 필요 (`BoardController` — 이메일 인증은 수정/삭제 시)
- 실종 제보 **작성**: 이메일 인증 필수 (`MissingPetBoardService`)
- 블라인드 처리: 관리자만 가능 (`AdminBoardController`: `@PreAuthorize("hasAnyRole('ADMIN','MASTER')")`)

### 2. 데이터 검증
- 위치 정보 검증: 위도/경도 범위 확인
- 파일 첨부 검증: 파일 크기, 확장자 제한
- 중복 신고 방지: Unique 제약조건

### 3. 입력 검증
- SQL Injection 방지 (JPA 사용)
- XSS 방지 (DTO 변환 시 이스케이프)
- 파일 경로 정규화 (Directory Traversal 방지)

## 📝 주요 API 엔드포인트

### 커뮤니티 게시판 (`BoardController` — `/api/boards`)
```
GET  /api/boards?category=&page=&size=        → BoardPageResponseDTO
GET  /api/boards/{id}?viewerId=              → BoardDTO
GET  /api/boards/search?keyword=&searchType=&page=&size=
GET  /api/boards/my-posts?userId=
GET  /api/boards/popular?period=WEEKLY|MONTHLY → List<BoardPopularitySnapshotDTO>

POST   /api/boards                            → BoardDTO (인증)
PUT    /api/boards/{id}                      → BoardDTO
DELETE /api/boards/{id}                      → 204

GET  /api/boards/{boardId}/comments?page=&size=
POST /api/boards/{boardId}/comments
DELETE /api/boards/{boardId}/comments/{commentId}

POST /api/boards/{boardId}/reactions
POST /api/boards/{boardId}/comments/{commentId}/reactions
```

### 실종 제보 (`MissingPetBoardController` — `/api/missing-pets`)
```
GET    /api/missing-pets?status=&page=&size=     → MissingPetBoardPageResponseDTO
GET    /api/missing-pets/{id}?commentPage=&commentSize=
GET    /api/missing-pets/{id}/comments?page=&size=
POST   /api/missing-pets                         → MissingPetBoardDTO
PUT    /api/missing-pets/{id}
PATCH  /api/missing-pets/{id}/status            body: { "status": "MISSING|FOUND|RESOLVED" }
DELETE /api/missing-pets/{id}                    → { "success": true }

POST   /api/missing-pets/{id}/comments
DELETE /api/missing-pets/{boardId}/comments/{commentId}

POST   /api/missing-pets/{boardIdx}/start-chat?witnessId=  → ConversationDTO (인증, 채팅)
```

### 관리자 (`AdminBoardController` — `/api/admin/boards`, `ADMIN`/`MASTER`)
```
GET   /api/admin/boards/paging?status=&deleted=&category=&q=&page=&size=
GET   /api/admin/boards/{id}

PATCH /api/admin/boards/{id}/blind
PATCH /api/admin/boards/{id}/unblind
POST  /api/admin/boards/{id}/delete
POST  /api/admin/boards/{id}/restore

GET   /api/admin/boards/{boardId}/comments?status=&deleted=
PATCH /api/admin/boards/{boardId}/comments/{commentId}/blind
PATCH /api/admin/boards/{boardId}/comments/{commentId}/unblind
POST  /api/admin/boards/{boardId}/comments/{commentId}/delete
POST  /api/admin/boards/{boardId}/comments/{commentId}/restore
```

## 🎯 핵심 포인트 요약

### 1. Magazine + Smart Grid 레이아웃
- **크기별 분류**: 공지사항/이미지 게시글 → 대형, 이미지 게시글 → 중형, 텍스트 게시글 → 소형
- **그리드 시스템**: CSS Grid로 반응형 레이아웃 구현
- **효과**: 시각적으로 매력적인 레이아웃, 공지사항 강조

### 2. 인기 게시글 추적
- **점수 공식**: (좋아요 × 3) + (댓글 × 2) + 조회수
- **스냅샷 생성**: 주간(매일 18:30), 월간(매주 월요일 18:30)
- **다단계 조회**: 정확한 날짜 매칭 → 겹치는 기간 → 최근 스냅샷 → 생성
- **효과**: 실시간 계산 부하 감소, 빠른 조회

### 3. 블라인드 처리
- **수동 처리**: 관리자가 `ContentStatus.BLINDED`로 상태 변경
- **현재 상태**: 신고 누적 시 자동 블라인드 처리는 구현되지 않음
- **효과**: 유해 콘텐츠 차단

### 4. 실종 골든타임 위치 공유
- **위치 저장**: 위도/경도 (`BigDecimal`, precision=15, scale=12)
- **지도 표시**: 지도에 마커로 위치 표시
- **반경·근처**: UI/지도에서 활용 가능; 백엔드 전용 반경 검색 API는 선택 사항
- **효과**: 직관적인 위치 공유, 골든타임 활용 극대화

### 5. 배치 조회 최적화
- **N+1 문제 해결**: 배치 조회로 반응/첨부파일 한 번에 조회
- **500개 단위 배치**: IN 절 크기 제한 회피
- **효과**: 쿼리 수 대폭 감소, 응답 시간 단축

### 6. 성능 최적화
- **스냅샷 기반**: 실시간 계산 대신 주기적 스냅샷 생성
- **배치 조회**: N+1 문제 해결
- **인덱스 전략**: 카테고리, 상태, 위치 기반 인덱스
