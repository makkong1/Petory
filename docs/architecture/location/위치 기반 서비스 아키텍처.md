...
| 탭         | 호출 API                                                            | limit 정책      |
| ---------- | ------------------------------------------------------------------- | --------------- |
| 주변서비스 | `locationServiceApi.searchPlaces` → `/api/location-services/search` | 백엔드 `NearbySearchPolicy`(반경 기준) |
| 모임       | `meetupApi.getNearbyMeetups` → `/api/meetups/nearby`                | 백엔드 `NearbySearchPolicy`(반경 기준) |
| 펫케어     | `careRequestApi.getNearby` → `/api/care-requests/nearby`            | 백엔드 `NearbySearchPolicy`(반경 기준) |

세 탭 모두 **프론트는 상한을 보내지 않는다**(2026-07-31 통일). 예전에는 주변서비스가 `size=300` 고정,
모임/펫케어가 `mapLevel` 기반이라 정책이 둘로 갈려 있었다.
...
| `radius`               | UI 반경, km 단위                                                          |
| `mapLevel`             | 지도 줌 레벨. **조회 파라미터가 아니다** — 지도 컴포넌트 표시용으로만 쓴다 |
...
sequenceDiagram
  ...
  UM->>API: GET /api/location-services/search?latitude&longitude&radius(m)&sort
...
메인 지도가 읍면동/도로명 직접 파라미터 대신 geocoding으로 좌표를 얻어 반경 검색을 하도록 바뀐 뒤로
이 두 메서드는 3계층(`SpringDataJpa*` / `*Repository` / `*Adapter`)에 **주석 처리된 채 방치**돼 있었다.
전용 인덱스 2개(`idx_locationservice_eupmyeondong_deleted_rating`, `idx_road_name_deleted_rating`)도
읽기 0회였다. 주석 코드와 인덱스를 함께 정리했다. 복구 DDL은 `V11` 마이그레이션 주석에 있다.
...
**Location의 기본 정렬은 `distance`(거리순)다.** 2026-07-31 지도 반경검색 통일로 care·meetup과 맞췄다.

그전에는 프론트가 `stable`을 명시적으로 보냈는데, 백엔드 `DEFAULT_RADIUS_SORT`는 이미 `distance`라
같은 API의 기본값이 두 개였다. 지도에서는 "가까운 순"이 기본으로 맞다고 보고 `distance`로 통일했다.

`stable`(추천순)을 기본으로 뒀던 원래 이유는 아래와 같고, 이건 **정렬 옵션으로 여전히 유효하다**.

- 거리순은 중심 좌표가 조금만 달라져도 절단선 근처 결과가 바뀐다.
- 기본 조회에서는 결과 안정성이 더 중요하다는 판단이었다.

→ 대신 **모든 정렬에 `idx ASC` 동점 처리**를 두어, 같은 중심·반경이면 순서가 결정적이 되게 했다.
동점 처리는 care·meetup에도 같이 적용했다.

현재 정책:

- 프론트·백엔드 기본값: `distance` (UI 라벨 "거리순")
- 결과 상한: **백엔드 `NearbySearchPolicy`가 반경으로 결정**(2km 100 / 5km 200 / 10km 350 / 20km 500 / 그 외 800).
  프론트는 상한을 보내지 않는다.
- Location 캐시 키: `type`, `lat`, `lng`, `radius`, `keyword`, `category`, `sort`
- **캐시 키에서 `mapLevel` 제외 — 이제 네 도메인 전부 그렇다.** 상한이 반경으로 결정되므로
  같은 중심·반경이면 줌을 바꿔도 결과가 같다. 예전에는 Meetup/Care가 캐시 키에 `mapLevel`을 넣어
  **줌만 바꿔도 캐시가 빗나가 재조회**했다.
