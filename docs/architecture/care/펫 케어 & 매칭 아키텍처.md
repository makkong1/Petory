...
FULLTEXT 인덱스는 `ngram` 파서를 사용한다(V9). 기본 InnoDB 파서는 2글자 미만 토큰(`innodb_ft_min_token_size=3`)을 색인하지 못하고 공백 기준 토큰화라 한글 부분 검색에 약하다. ngram(2글자 분해)이 이를 보완하며 board·meetup과 파서를 통일한다. `ngram_token_size`는 서버 전역 설정(기본 2)이라 인덱스별 지정은 불가하다. 이력상 V2가 검색 500(FULLTEXT 부재)을 막은 핫픽스였고, ngram 전환은 V9에서 근본 개선으로 분리했다.
