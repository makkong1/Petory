...
graph TB
    ...
    subgraph "Backend API Layer"
        ...
        CRCC[CareRequestCommentController<br/>/api/care-requests/.../comments]
...
- `getCareRequestsWithPaging()` / `getAllCareRequests()`: 목록 조회(페이징·필터는 컨트롤러가 전자 사용)
- `updateStatus()`: 상태 변경 (OPEN → IN_PROGRESS → COMPLETED → CANCELLED)
- `searchCareRequestsWithPaging()` / `searchCareRequests()`: 키워드 검색(페이징 유무)
...
5. 양쪽 모두 확정 시:
   ...
   - **펫코인 차감 및 에스크로 생성** (요청자가 설정한 `offeredCoins`만큼)
     ...
     - **구현 주의**: `createEscrow` 예외 시 현재 코드는 **로그만 남기고 거래 확정(상태 전이)은 유지** — 운영 정책에 따라 롤백 여부를 재검토할 여지가 있음
...
**핵심 로직** (실제 `CareRequestScheduler`):
```java
List<CareRequest> expiredRequests = careRequestRepository.findByDateBeforeAndStatusIn(
    now, List.of(CareRequestStatus.OPEN, CareRequestStatus.IN_PROGRESS));

for (CareRequest request : expiredRequests) {
    careRequestService.updateStatus(request.getIdx(), "COMPLETED", null); // 에스크로·검증 포함
}
```
...
2. **제시 코인·잔액**
   - `offeredCoins`는 **null/0 이하 불가** (`CareValidationException.coinMustBePositive`)
   - 요청자 **펫코인 잔액 ≥ offeredCoins** (`insufficientBalance`)
...
- 제시 코인은 양수·잔액 검증(시간당 최소 코인 같은 **세부 가이드는 별도 정책 문서** — 서버에 시간·체급별 하한 공식은 없음)
...
3. **중복 리뷰 확인**
   ...
   - 중복이면 `CareConflictException.alreadyReviewed()`
...
3. **펫코인 지급 처리** (상태가 `COMPLETED`로 변경될 때)
   - 에스크로 조회 (`PetCoinEscrowService.findByCareRequestForUpdate` — 비관적 락)
   - 에스크로 상태가 `HOLD`인 경우:
     - 제공자에게 코인 지급 (`PetCoinEscrowService.releaseToProvider`)
     - 지급 실패 시 `CarePaymentException.paymentFailed`로 **상태 변경 롤백**
     - 상세 내용은 [Payment 도메인 문서](../domains/payment.md) 참조
...
3. **상태 변경**
   - 요청마다 `CareRequestService.updateStatus(..., "COMPLETED", null)` 호출 (에스크로·완료 처리 로직 포함)
   - 실패 건은 로그만 남기고 다음 건 계속 처리

**특징:**
- 자동 완료로 사용자 개입 없이 처리
- 서비스 메서드 경로로 상태 변경해 도메인 규칙·결제 연동 일관성 유지
...
sequenceDiagram
    ...
    Frontend->>ConversationService: POST /api/chat/conversations/.../confirm-deal?userId=
    ConversationService->>ConversationService: confirmCareDeal() — dealConfirmed 설정
    
    Requester->>Frontend: 거래 확정 버튼 클릭
    Frontend->>ConversationService: POST .../confirm-deal (동일 API)
...
sequenceDiagram
    ...
    participant Svc as CareRequestService
    participant MySQL
    
    Note over Scheduler: 매 시간 정각 / 매일 자정
    Scheduler->>MySQL: 만료 후보 조회 (date &lt; now, OPEN·IN_PROGRESS)
    MySQL-->>Scheduler: CareRequest 목록
    
    loop 각 요청마다
        Scheduler->>Svc: updateStatus(id, COMPLETED, null)
        Svc->>MySQL: 상태·completedAt·에스크로 지급 등
    end
```
...
#### 배치 처리
- 스케줄러는 만료 건마다 `updateStatus` 호출(도메인 로직 일원화). 단순 `saveAll` 일괄 갱신과는 다름
...
### 케어 요청 (`CareRequestController` — `/api/care-requests`)
```
GET    /api/care-requests/nearby?lat=&lng=&radius=&limit=   → List (지도, 기본 radius=5km, limit=200, 서비스 상한 500)
GET    /api/care-requests?status=&location=&page=&size=     → CareRequestPageResponseDTO
GET    /api/care-requests/{id}                              → CareRequestDTO
POST   /api/care-requests                                   → CareRequestDTO
PUT    /api/care-requests/{id}                              → CareRequestDTO (본인)
DELETE /api/care-requests/{id}                             → 204

GET    /api/care-requests/my-requests?userId=
PATCH  /api/care-requests/{id}/status?status=
GET    /api/care-requests/search?keyword=&page=&size=     → CareRequestPageResponseDTO
```

### 리뷰 (`CareReviewController` — `/api/care-reviews`)
```
POST /api/care-reviews
GET  /api/care-reviews/reviewee/{revieweeIdx}
GET  /api/care-reviews/reviewer/{reviewerIdx}
GET  /api/care-reviews/average-rating/{revieweeIdx}       → Double (평균 평점)
```

### 케어 요청 댓글 (`CareRequestCommentController` — `/api/care-requests/{careRequestId}/comments`)
```
GET    /api/care-requests/{careRequestId}/comments         → List<CareRequestCommentDTO>
POST   /api/care-requests/{careRequestId}/comments
DELETE /api/care-requests/{careRequestId}/comments/{commentId}  → 204
```

### 채팅 — 펫케어 거래 확정 (`ConversationController` — `/api/chat/conversations`)
```
POST /api/chat/conversations/{conversationIdx}/confirm-deal?userId=   → 204 (양쪽 확정 시 매칭·에스크로 등은 ConversationService.confirmCareDeal)
```
