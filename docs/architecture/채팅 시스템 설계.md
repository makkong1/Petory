...
-- Full-Text 인덱스 (검색용). 운영·indexes.sql 기준명: idx_chat_message_content
CREATE FULLTEXT INDEX idx_chat_message_content
ON chatmessage(content) WITH PARSER ngram;
```
...
4. **메시지 검색**
   ```sql
   -- Full-Text 인덱스 (단일 인덱스만 유지: idx_chat_message_content)
   CREATE FULLTEXT INDEX idx_chat_message_content
   ON chatmessage(content) WITH PARSER ngram;
   ```
