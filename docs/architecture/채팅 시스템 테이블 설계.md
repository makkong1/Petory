...
4. **채팅방 생성**
   - 승인된 지원자와 보호자 간 1:1 채팅방(`Conversation`) 생성.
...
3. **참여자 추가**
   - 모임 참여자(`MeetupParticipants`)가 추가될 때마다 `ConversationParticipant`에도 추가.
   - 모임 주최자는 자동으로 `ADMIN` 역할.
...
- `requester_unread_count`, `provider_unread_count`: 읽지 않은 메시지 수 캐싱
