...
- **참고**: `LocationRecommendAgentService`(Spring AI/Ollama 기반 AI 추천)와 `GET /api/location-services/recommend` 엔드포인트는 **이미 제거됨**. 추천 기능은 별도 Python 서비스(pet-data-api)의 `GET /api/recommend`로 단일화. 상세는 [pet-data-api architecture](./pet-data-api%20architecture.md).
