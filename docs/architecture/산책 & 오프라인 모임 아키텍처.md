...
- 과거 구현은 전체 조회 후 인메모리 Haversine이었으나, **현재는 `MeetupRepository.findNearbyMeetups(lat, lng, radius, now)`** 한 번에 처리합니다.
- **Bounding box**(위도·경도 범위)로 1차 축소 후, SQL에서 Haversine 거리(km) ≤ `radius` 조건 및 `ORDER BY` 거리·날짜.
- `date > :currentDate`, `status != COMPLETED`, 소프트 삭제 제외는 모두 쿼리 조건에 포함.

구현 참고: `SpringDataJpaMeetupRepository.findNearbyMeetups` (nativeQuery = true).
...
- 참여 취소 시: 일반 참가자만 삭제 가능, `currentParticipants`는 `max(0, n-1)`로 감소.
...
1. **`meetupRepository.findNearbyMeetups(lat, lng, radius, now)` 호출**
   - Native SQL: 위도·경도 **bounding box**로 후보 축소 후 Haversine 거리(km) ≤ 반경
   - `date > now`, `COMPLETED` 제외, 소프트 삭제 제외
   - 정렬: 거리 오름차순, 동일 시 `date` 오름차순

2. **DTO 변환**
   - `convertToDTOs`로 `List<MeetupDTO>` 반환

**특징:**
- 인메모리 전체 스캔 방식은 제거되었고, **DB 단일 쿼리**로 반경 검색을 수행 (리팩토링 반영).
...
sequenceDiagram
    ...
    MeetupService->>MySQL: findNearbyMeetups (bounding box + Haversine, native)
    MySQL-->>MeetupService: 필터·정렬된 Meetup 목록
...
#### 반경 기반 검색 최적화
- **DB Native 쿼리**: Bounding box + Haversine을 한 번에 처리 (`findNearbyMeetups`)
- 서비스 레이어는 결과 DTO 변환만 수행
...
- 모임 수정·삭제: 컨트롤러에서 **주최자 전용 검증은 없음** (운영·보안상 보강 여지)
...
### 모임 검색·목록
```
GET /api/meetups/nearby?lat=&lng=&radius=   (기본 radius=5.0 km)
    → { meetups, count }
