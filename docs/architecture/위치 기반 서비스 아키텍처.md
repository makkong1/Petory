...
graph TB
    ...
    subgraph "Backend API Layer"
        ...
        ALC[AdminLocationController<br/>/api/admin/location-services]
    ...
    subgraph "Service Layer"
        ...
        LSAS[LocationServiceAdminService<br/>초기 데이터 로드]
        PDLS[PublicDataLocationService<br/>CSV 임포트]
    end
...
**주요 메서드**:
- `searchLocationServicesByKeyword()`: 키워드 검색 (이름·주소·카테고리 등, Soft Delete 제외)
- `searchLocationServicesByLocation()`: 위도·경도·반경(m)으로 DB 반경 조회 후 DTO에 거리(m) 부여 (`calculateDistance`)
- `searchLocationServicesByRegion()`: 시도/시군구/읍면동/도로명 계층 검색
- 컨트롤러 `GET /search`는 위 메서드들을 **키워드 → (위도·경도·반경 모두 있음) 위치 검색 → 그 외 지역 검색** 순으로 호출
- `GET /recommend`는 동일 분기로 후보를 만든 뒤 `LocationRecommendAgentService.enrichWithRecommendations()`를 호출(컨트롤러에서 조합)
- `deleteService()`: Soft Delete (`deletedAt`)
...
#### 컨트롤러 검색 분기 (`GET /search`)
1. `keyword`가 있으면 → 키워드 검색
2. `latitude`, `longitude`, `radius`가 모두 있으면 → 반경 검색(DB `findByRadius` 등)
...
**컨트롤러 분기** (`LocationServiceController.searchLocationServices`):

1. **`size` (effectiveSize)**
   - 생략 시 **기본 100건**
   - `size <= 0`이면 **상한 없음**(서비스에 `null` 전달, 전체에 가까운 조회 가능)

2. **키워드 우선** (`keyword` 비어 있지 않음)
   - `searchLocationServicesByKeyword(keyword, category, effectiveSize)`

3. **위치·반경** (`latitude`, `longitude`, `radius` 모두 존재)
   - `radius` ≤ 0이면 기본 **10000m**
   - `searchLocationServicesByLocation(...)` — DB 반경 조회 후 거리 필드 채움
...
**카테고리 필터링** (지역/키워드 경로에서):
- `category3` → `category2` → `category1` 순, 대소문자 무시

**특징:**
- “초기 로드만 지역 검색”이 아니라, **키워드·반경·지역** 중 요청에 맞는 경로로 동작
...
- 반경 필터링은 **저장소 `findByRadius`** — Haversine은 표시용 거리
...
### 3. 권한 제어
- 리뷰 작성: 이메일 인증 필수
- 관리자 위치 서비스: `AdminLocationController` — 목록 조회는 `ADMIN`/`MASTER`, 초기 데이터 로드·CSV 임포트 등은 코드상 **`MASTER` 전용** 메서드가 있음(각 `@PreAuthorize` 확인)

## 📝 주요 API 엔드포인트

**인증**: `SecurityConfig` 기준 `GET /api/geocoding/**`는 공개(permitAll). **`/api/location-services/**`, `/api/location-service-reviews/**` 등은 로그인(authenticated) 필요.** 관리자는 `/api/admin/**`에 `ADMIN`/`MASTER`, 그 안에서 일부는 `@PreAuthorize`로 `MASTER`만.
...
```
GET  /api/admin/location-services          # ADMIN, MASTER — 목록·q 검색
POST /api/admin/location-services/load-data              # MASTER — 초기 데이터 로드
POST /api/admin/location-services/import-public-data     # MASTER — CSV 업로드
POST /api/admin/location-services/import-public-data-path # MASTER — 경로 임포트
```

## 🎯 핵심 포인트 요약

### 1. 하이브리드 검색·상한
- **분기**: 키워드 → 위도·경도·반경 → 지역(`searchLocationServicesByRegion`)
- **`size`**: 기본 100, `<= 0`이면 무제한(대량 응답 가능)
...
### 3. 거리·반경
- **Haversine**: DTO 거리(m) 표시
- **반경 필터**: DB `findByRadius` 계열
