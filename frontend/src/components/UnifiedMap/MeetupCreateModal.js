...
const MeetupCreateModal = ({ onClose, onSuccess }) => {
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => ({ ...prev, [name]: name === 'maxParticipants' ? Number(value) : value }));
  ...
  const validate = () => {
    ...
    if (!formData.latitude || !formData.longitude) e.location = '주소 검색 후 결과를 선택해주세요.';
    if (!formData.date) e.date = '날짜/시간을 입력해주세요.';
    else if (new Date(formData.date) < new Date()) e.date = '현재 이후 시간을 입력해주세요.';
    if (!formData.maxParticipants || formData.maxParticipants < 1) e.maxParticipants = '1명 이상이어야 합니다.';
    setErrors(e);
    ...
    <Overlay onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      <Modal>
        ...
        <Form onSubmit={handleSubmit}>
          <Field>
            <Label>제목 *</Label>
            <Input name="title" value={formData.title} onChange={handleChange} placeholder="모임 이름을 입력하세요" />
          ...
          <Field>
            ...
            <Input
              ...
              max="100"
            />
            {errors.maxParticipants && <ErrorMsg>{errors.maxParticipants}</ErrorMsg>}
          </Field>
