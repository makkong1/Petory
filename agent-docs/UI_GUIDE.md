...
## 14. AI 에이전트 코딩 가이드라인 (AI 슬롭 안티패턴 방지)

⚠️ **프론트엔드 작업 시 AI가 무분별하게 추가하는 기본 패턴을 강력히 억제합니다.**

- **Glass morphism (글래스모피즘) 남용 금지**: 배경 흐림(`backdrop-filter: blur`), 과도한 반투명 카드 사용을 피하세요.
- **보라색/파란색 그라데이션 텍스트 금지**: 명확한 텍스트 가독성을 훼손하는 텍스트 그라데이션을 사용하지 마세요. (브랜드 컬러 팔레트에 지정된 색상만 사용).
- **네온 글로우 효과 지양**: 과장된 Box-shadow나 Text-shadow 발광 효과를 넣지 마세요 (`box-shadow` 토큰에 정의된 그림자만 사용).
- **불필요한 마이크로 애니메이션 억제**: 기본으로 지정된 `duration` 토큰 규격을 벗어난 복잡한 bounce, jiggle 액션을 임의로 넣지 마세요.
- 오직 이 문서에 정의된 Theme 컬러 조각, Spacing, Typography 규격 내에서만 UI를 구성해야 합니다.
